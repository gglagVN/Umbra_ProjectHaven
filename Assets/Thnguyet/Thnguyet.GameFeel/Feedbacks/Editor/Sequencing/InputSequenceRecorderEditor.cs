...
namespace Thnguyet.GameFeel.Feedbacks
{
	...
	/// </summary>
	[CustomEditor(typeof(InputSequenceRecorder), true)]
	[CanEditMultipleObjects]
	public class InputSequenceRecorderEditor : Editor
	{
		...
		protected InputSequenceRecorder _targetRecorder;
		...
		protected virtual void OnEnable()
		{
			...
			_targetRecorder = (InputSequenceRecorder)target;
		...
		protected virtual void EditorDetectRecording()
		{
			if (_targetRecorder.Recording && (_targetRecorder.SequenceScriptableObject != null))
			{
				if (_currentEvent.isKey)
				{
					foreach (SequenceTrack track in _targetRecorder.SequenceScriptableObject.SequenceTracks)
					{
						if (_currentEvent.keyCode == (track.Key))
						{
							if (track.State == SequenceTrackStates.Up)
							{
								track.State = SequenceTrackStates.Idle;
							}
							if (_currentEvent.type == EventType.KeyDown)
							{
								if (track.State != SequenceTrackStates.Down)
								{
									// key is down for the first time
									_targetRecorder.AddNoteToTrack(track);
								}
								track.State = SequenceTrackStates.Down;
							}
							if (_currentEvent.type == EventType.KeyUp)
							{
								// key is up
								track.State = SequenceTrackStates.Up;
							}
						}
					}
				}
