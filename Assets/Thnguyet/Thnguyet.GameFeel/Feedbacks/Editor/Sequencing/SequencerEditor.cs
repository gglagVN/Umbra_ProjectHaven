...
{
	/// <summary>
	/// Custom editor for FeelSequencer, handles recalibration and sequencer display
	/// </summary>
	[CustomEditor(typeof(FeelSequencer), true)]
	[CanEditMultipleObjects]
	public class SequencerEditor : Editor
	{
		protected FeelSequencer _targetSequencer;
		...
		{
			_targetSequencer = (FeelSequencer)target;
		...
		protected virtual void FillControlList()
		{
			...
			if (_targetSequencer.Sequence != null)
			{
				...
				foreach (SequenceTrack track in _targetSequencer.Sequence.SequenceTracks)
				{
		...
		protected virtual void DrawTrack(int trackIndex)
		{
			...
			if (GUILayout.Button("", _trackControlStyle, GUILayout.Width(_trackControlWidth), GUILayout.Height(_trackControlWidth)))
			{
				...
				{
					_targetSequencer.ApplyToggleActive(trackIndex);
					EditorUtility.SetDirty(_targetSequencer.Sequence);
				}
			}
