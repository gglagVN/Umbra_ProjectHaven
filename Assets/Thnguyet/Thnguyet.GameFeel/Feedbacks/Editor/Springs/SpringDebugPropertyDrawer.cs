...
{
	[CustomPropertyDrawer(typeof(SpringDebug))]
	public class SpringDebugPropertyDrawer : PropertyDrawer
	{
		protected Color _backgroundBarColor = new Color(0f, 0f, 0f, 0.3f);
		protected Color _frontBarColor = FeelColors.Yellow;
		...
		public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
		{
			...
			if (Application.isPlaying)
			{
				...
				normalizedValue = FeelMaths.Remap(diff, -_max, _max, -1f, 1f);	

				float newWidth = FeelMaths.Remap(normalizedValue, -1f, 1f, -position.width/2f, position.width/2f);
