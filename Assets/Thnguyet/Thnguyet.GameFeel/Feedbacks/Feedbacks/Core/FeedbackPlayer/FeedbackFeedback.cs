...
{
	[Serializable]
	public abstract class FeedbackFeedback
	{
		#region Properties
		...
		[FeedbackInspectorGroup("Feedback Settings", true, 0, false, true)]
		...
		public string Label = "FeelFeedback";
		...
		/// the FeelChannel definition asset to use to broadcast this feedback. The shaker will have to reference that same FeelChannel definition to receive events - to create a FeelChannel,
		...
		/// use this color to customize the background color of the feedback in the FeedbackPlayer's list
		[Tooltip("use this color to customize the background color of the feedback in the FeedbackPlayer's list")]
		...
		public FeedbackTargetAcquisition AutomatedTargetAcquisition;
		
		[FeedbackInspectorGroup(_randomnessGroupName, true, 58, false, true)]
		...
		[Tooltip(
			"a random value (randomized between its x and y) by which to multiply the output of this feedback, if RandomizeOutput is true")]
		[FeedbackCondition("RandomizeOutput", true)]
		...
		[Tooltip(
			"a random value (randomized between its x and y) by which to multiply the duration of this feedback, if RandomizeDuration is true")]
		[FeedbackCondition("RandomizeDuration", true)]
		[FeedbackVector("Min", "Max")]
		public Vector2 RandomDurationMultiplier = new Vector2(0.5f, 2f);

		[FeedbackInspectorGroup(_rangeGroupName, true, 47)]
		...
		[FeedbackInspectorGroup(_automaticSetupGroupName, true, 49, false, true)]
		...
		public FeedbackButton AutomaticShakerSetupButton;
		...
		/// if this is true, this feedback will pause and wait until ResumeFeedbacks() is called on its parent FeedbackPlayer to resume execution
		...
		public virtual FeedbackReferenceHolder ForcedReferenceHolder { get; set; }
		...
		public virtual void CacheRequiresSetup()
		{
			...
			if (_requiresSetup && HasAutomatedTargetAcquisition && (AutomatedTargetAcquisition != null) && (AutomatedTargetAcquisition.Mode != FeedbackTargetAcquisition.Modes.None))
			{
		...
		/// if this is true, the feedback will be displayed in the Feedback Player's list with a full color background, as opposed to just a small line on the left
		...
		public virtual string RequiredChannelText
		{
			get
			{
				if (ChannelMode == ChannelModes.FeelChannel)
				{
		...
		public virtual ChannelData ChannelData => _channelData.Set(ChannelMode, Channel, ChannelDefinition);
		...
		{
			_channelData = new ChannelData(ChannelMode, Channel, ChannelDefinition);
		}
		...
		protected virtual void AutomateTargetAcquisitionInternal()
		{
			...
			{
				AutomatedTargetAcquisition = new FeedbackTargetAcquisition();
			}

			if (AutomatedTargetAcquisition.Mode == FeedbackTargetAcquisition.Modes.None)
			{
				return;
			}
		...
		{
			return FeedbackTargetAcquisition.FindAutomatedTargetGameObject(AutomatedTargetAcquisition, Owner, _indexInOwnerFeedbackList);
		}
		...
		{
			return FeedbackTargetAcquisition.FindAutomatedTarget<T>(AutomatedTargetAcquisition, Owner, _indexInOwnerFeedbackList);
		}
		...
		protected virtual IEnumerator SequenceCoroutine(Vector3 position, float feedbacksIntensity = 1.0f)
		{
			...
			else
			{
				...
				{
					foreach (SequenceNote item in Timing.Sequence.OriginalSequence.Line)
					{
		...
		public virtual void SetSequence(FeelSequence newSequence)
		{
		...
		/// <summary>
		/// Called when skipping to the end of FeedbackPlayer, calls custom Skip on all feedbacks
		/// </summary>
		/// <param name="position"></param>
		/// <param name="feedbacksIntensity"></param>
		...
		/// <summary>
		/// This gets called by the Feedback Player when all feedbacks have completed playing 
		/// </summary>
		...
		/// <summary>
		/// Applies the host FeelFeedbacks' time multiplier to this feedback
		/// </summary>
		...
		public virtual bool NormalPlayDirection
		{
			get
			{
				...
				{
					case FeedbackTiming.PlayDirections.FollowMMFeedbacksDirection:
						return (Owner.Direction == FeedbackPlayer.Directions.TopToBottom);
					case FeedbackTiming.PlayDirections.AlwaysNormal:
						return true;
					case FeedbackTiming.PlayDirections.AlwaysRewind:
						return false;
					case FeedbackTiming.PlayDirections.OppositeMMFeedbacksDirection:
						return !(Owner.Direction == FeedbackPlayer.Directions.TopToBottom);
				}
		...
		/// <summary>
		/// Returns true if this feedback should play in the current parent FeelFeedbacks direction, according to its FeedbacksDirectionCondition setting
		/// </summary>
		public virtual bool ShouldPlayInThisSequenceDirection
		{
			get
			{
				...
				switch (Timing.FeedbacksDirectionCondition)
				{
					case FeedbackTiming.FeedbacksDirectionConditions.Always:
						return true;
					case FeedbackTiming.FeedbacksDirectionConditions.OnlyWhenForwards:
						return (Owner.Direction == FeedbackPlayer.Directions.TopToBottom);
					case FeedbackTiming.FeedbacksDirectionConditions.OnlyWhenBackwards:
						return (Owner.Direction == FeedbackPlayer.Directions.BottomToTop);
				}
		...
		public virtual void InitializeCustomAttributes()
		{
			...
			{
				AutomaticShakerSetupButton = new FeedbackButton("Automatic Shaker Setup", AutomaticShakerSetup);
			}
		}
		...
		/// <summary>
		/// Triggered when the host Feedback Player gets disabled
		/// </summary>
		public virtual void OnDisable() { }

		/// <summary>
		/// Triggered when the host Feedback Player gets selected, can be used to draw gizmos
		/// </summary>
