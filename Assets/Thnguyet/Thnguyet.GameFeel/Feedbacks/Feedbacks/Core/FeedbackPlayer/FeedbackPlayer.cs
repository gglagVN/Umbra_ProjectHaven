...
{
	public struct FeedbackPlayerEvent
	{
		public enum Modes
		{
			PlayFeedbacks,
		...
		public static void Trigger(ChannelData channelData, bool usePosition, Vector3 position, Modes mode = Modes.PlayFeedbacks, float feedbacksIntensity = 1.0f, bool forceChangeDirection = false)
		{
	...
	[AddComponentMenu("Thnguyet/GameFeel/Feedbacks/Feedback Player")]
	[DisallowMultipleComponent] 
	public class FeedbackPlayer : FeelFeedbacks, EventListener<FeedbackPlayerEvent>
	{
		#region PROPERTIES
        
		[SerializeReference]
		public List<FeedbackFeedback> FeedbacksList;
		...
		/// you can use FeedbackPlayerEvents to trigger any Feedback Player listening on the corresponding FeelChannel
		/// This field lets you define whether this Feedback Player should listen on a channel defined by an int or by a FeelChannel scriptable object.
		...
		[Tooltip("you can use FeedbackPlayerEvents to trigger any Feedback Player listening on the corresponding FeelChannel. " +
		         "This field lets you define whether this Feedback Player should listen on a channel defined by an int or by a FeelChannel scriptable object." +
		         "Ints are simple to setup but can get messy and make it harder to remember what int corresponds to what." +
		         "FeelChannel scriptable objects require you to create them in advance, but come with a readable name and are more scalable")]
		public ChannelModes FeedbackChannelMode = ChannelModes.Int;
		...
		[EnumCondition("FeedbackChannelMode", (int)ChannelModes.Int)]
		public int FeedbackChannel = 0;
		...
		[Tooltip("the FeelChannel definition asset to use to listen for events. The feedbacks targeting this shaker will have to reference that same FeelChannel definition to receive events - to create a FeelChannel, " +
		         "right click anywhere in your project (usually in a Data folder) and go Thnguyet > GameFeel > FeelChannel, then name it with some unique name")]
		[EnumCondition("FeedbackChannelMode", (int)ChannelModes.FeelChannel)]
		public FeelChannel FeedbackChannelDefinition = null;
		...
		{
			this.EventStartListening<FeedbackPlayerEvent>();
			...
			// if our FeelFeedbacks is in AutoPlayOnEnable mode, we add a little helper to it that will re-enable it if needed if the parent game object gets turned off and on again
			if (AutoPlayOnEnable)
			{
				FeedbackPlayerEnabler playerEnabler = GetComponent<FeedbackPlayerEnabler>(); 
				if (playerEnabler == null)
				{
					playerEnabler = this.gameObject.AddComponent<FeedbackPlayerEnabler>();
				}
		...
		protected virtual void InitializeFeedbackList()
		{
			...
			{
				FeedbacksList = new List<FeedbackFeedback>();
			}
		}
		...
		{	
			...
			{
				SetFeedbackRangeCenterEvent.Register(OnMMSetFeedbackRangeCenterEvent);	
			}
			foreach (FeedbackFeedback feedback in FeedbacksList)
			{
		...
		{
			yield return FeedbacksCoroutine.WaitForFrames(framesAmount);
			PlayFeedbacks();
		}
		...
		/// <summary>
		/// When calling the legacy init method that used to specify an owner, we force the Feedback Player init to run
		/// </summary>
		/// <param name="owner"></param>
		...
		/// <summary>
		/// Plays all feedbacks using the FeelFeedbacks' position as reference, and no attenuation
		/// </summary>
		...
		/// <summary>
		/// Plays all feedbacks using the FeelFeedbacks' position as reference, and no attenuation, and in reverse (from bottom to top)
		/// </summary>
		...
		/// <summary>
		/// Plays all feedbacks in the sequence, but only if this FeelFeedbacks is playing in reverse order
		/// </summary>
		...
		/// <summary>
		/// Plays all feedbacks in the sequence, but only if this FeelFeedbacks is playing in normal order
		/// </summary>
		...
		/// <summary>
		/// A public coroutine you can call externally when you want to yield in a coroutine of yours until the FeelFeedbacks has stopped playing
		/// typically : yield return myFeedback.PlayFeedbacksCoroutine(this.transform.position, 1.0f, false);
		/// </summary>
		/// <param name="position">The position at which the FeelFeedbacks should play</param>
		/// <param name="feedbacksIntensity">The intensity of the feedback</param>
		/// <param name="forceChangeDirection">Whether or not the FeelFeedbacks should play in reverse or not</param>
		/// <returns></returns>
		...
		public virtual bool IsAllowedToPlay(Vector3 position)
		{
			...
			// if all FeelFeedbacks are disabled globally, we're not allowed to play
		...
		protected virtual IEnumerator FrameOnePlayCo(Vector3 position, float feedbacksIntensity, bool forceChangeDirection = false)
		{
			...
			_lastStartAt = _startTime;
			IsPlaying = true;
			yield return FeedbacksCoroutine.WaitForUnscaled(ComputedInitialDelay);
			PreparePlay(position, feedbacksIntensity, forceChangeDirection);
		}
		...
		protected override IEnumerator HandleInitialDelayCo(Vector3 position, float feedbacksIntensity, bool forceChangeDirection = false)
		{
			...
			{
				yield return FeedbacksCoroutine.WaitFor(ComputedInitialDelay);
			}
			else
			{
				yield return FeedbacksCoroutine.WaitForUnscaled(ComputedInitialDelay);	
		...
		protected override IEnumerator PausedFeedbacksCo(Vector3 position, float feedbacksIntensity)
		{
			...
			while ((i >= 0) && (i < count))
			{
				...
				if ((FeedbacksList[i].LooperPause == true)
				    ...
				    && (FeedbacksList[i].ShouldPlayInThisSequenceDirection)
				    && (((FeedbacksList[i] as FeedbackLooper).NumberOfLoopsLeft > 0) || (FeedbacksList[i] as FeedbackLooper).InInfiniteLoop))
				{
					...
					bool loopAtLastPause = (FeedbacksList[i] as FeedbackLooper).LoopAtLastPause;
					bool loopAtLastLoopStart = (FeedbacksList[i] as FeedbackLooper).LoopAtLastLoopStart;
		...
		/// <summary>
		/// Changes the direction of this FeelFeedbacks
		/// </summary>
		...
		/// <summary>
		/// Resets the cooldowns on this Feedback Player, for the Feedback Player itself and for all its feedbacks
		/// </summary>
		...
		/// <summary>
		/// Resets the cooldown on this Feedback Player, letting you play it again instantly, regardless of its cooldown settings and how much time has passed since its last play
		/// </summary>
		...
		/// <summary>
		/// Adds the specified FeedbackFeedback to the player
		/// </summary>
		/// <param name="newFeedback"></param>
		public virtual void AddFeedback(FeedbackFeedback newFeedback, bool copy = false)
		{
			InitializeFeedbackList();
		...
		/// <returns></returns>
		public new FeedbackFeedback AddFeedback(System.Type feedbackType, bool add = true)
		{
			InitializeFeedbackList();
			...
			newFeedback.Owner = this;
			newFeedback.Timing = new FeedbackTiming();
			newFeedback.UniqueID = Guid.NewGuid().GetHashCode();
			if (add)
			{
				FeedbacksList.Add(newFeedback);	
		...
		private class FeedbackFeedbackListCopy
		{
			[SerializeReference] 
			public List<FeedbackFeedback> FeedbackList;

			/// <summary>
			/// Returns a list of feedbacks copied from the list on the specified source Feedback Player 
			/// </summary>
			/// <param name="source"></param>
			/// <returns></returns>
			public static List<FeedbackFeedback> CopyFrom(FeedbackPlayer source)
			{
				FeedbackFeedbackListCopy listCopy = new FeedbackFeedbackListCopy();
		...
		/// <summary>
		/// Replaces the current feedback list and player settings with the ones on the target Feedback Player
		/// </summary>
		/// <param name="source"></param>
		public virtual void CopyPlayerFrom(FeedbackPlayer source)
		{
			JsonUtility.FromJsonOverwrite(JsonUtility.ToJson(source), this);
		}

		/// <summary>
		/// Replaces the current feedback list with the feedbacks on the target Feedback Player
		/// </summary>
		/// <param name="source"></param>
		public virtual void CopyFeedbackListFrom(FeedbackPlayer source)
		{
			FeedbacksList = FeedbackFeedbackListCopy.CopyFrom(source);
		}

		/// <summary>
		/// Adds the feedbacks on the target Feedback Player to the current feedback list
		/// </summary>
		/// <param name="source"></param>
		public virtual void AddFeedbackListFrom(FeedbackPlayer source) 
		{
			List<FeedbackFeedback> tempList = new List<FeedbackFeedback>();
			List<FeedbackFeedback> tempList2 = new List<FeedbackFeedback>();
			
			tempList = FeedbackFeedbackListCopy.CopyFrom(this);
			tempList2 = FeedbackFeedbackListCopy.CopyFrom(source);
		...
		/// <summary>
		/// Returns true if one or more of the feedbacks on this Feedback Player have an option for automatic shaker setup, false otherwise
		/// </summary>
		...
		/// <summary>
		/// Checks whether or not this FeelFeedbacks contains one or more looper feedbacks
		/// </summary>
		...
		public virtual float ComputeRangeIntensityMultiplier(Vector3 position)
		{
			...
			float normalizedDistance = FeedbacksHelpers.Remap(distanceToCenter, 0f, RangeDistance, 0f, 1f);
			float curveValue = RangeFalloff.Evaluate(normalizedDistance);
			float newIntensity = FeedbacksHelpers.Remap(curveValue, 0f, 1f, RemapRangeFalloff.x, RemapRangeFalloff.y);
			return newIntensity;
		}
        
		/// <summary>
		/// This will return true if the conditions defined in the specified feedback's Timing section allow it to play in the current play direction of this FeelFeedbacks
		/// </summary>
		/// <param name="feedback"></param>
		/// <returns></returns>
		protected bool FeedbackCanPlay(FeedbackFeedback feedback)
		{
			if (feedback.Timing.FeedbacksDirectionCondition == FeedbackTiming.FeedbacksDirectionConditions.Always)
			{
				return true;
			}
			else if (((Direction == Directions.TopToBottom) && (feedback.Timing.FeedbacksDirectionCondition == FeedbackTiming.FeedbacksDirectionConditions.OnlyWhenForwards))
			         || ((Direction == Directions.BottomToTop) && (feedback.Timing.FeedbacksDirectionCondition == FeedbackTiming.FeedbacksDirectionConditions.OnlyWhenBackwards)))
			{
				return true;
			}
			return false;
		...
		/// <summary>
		/// Readies the FeelFeedbacks to change direction on the next play
		/// </summary>
		...
		/// <returns></returns>
		public virtual T GetFeedbackOfType<T>(AccessMethods method, int referenceIndex) where T:FeedbackFeedback
		{
			_t = typeof(T);
		...
		/// <summary>
		/// Returns the first feedback of the searched type on this FeedbackPlayer
		/// </summary>
		/// <typeparam name="T"></typeparam>
		/// <returns></returns>
		public virtual T GetFeedbackOfType<T>() where T:FeedbackFeedback
		{
			_t = typeof(T);
			foreach (FeedbackFeedback feedback in FeedbacksList)
			{
		...
		/// <summary>
		/// Returns a list of all the feedbacks of the searched type on this FeedbackPlayer
		/// </summary>
		/// <typeparam name="T"></typeparam>
		/// <returns></returns>
		public virtual List<T> GetFeedbacksOfType<T>() where T:FeedbackFeedback
		{
		...
		/// <typeparam name="T"></typeparam>
		/// <returns></returns>
		public virtual T GetFeedbackOfType<T>(string searchedLabel) where T:FeedbackFeedback
		{
			_t = typeof(T);
			foreach (FeedbackFeedback feedback in FeedbacksList)
			{
		...
		/// <typeparam name="T"></typeparam>
		/// <returns></returns>
		public virtual List<T> GetFeedbacksOfType<T>(string searchedLabel) where T:FeedbackFeedback
		{
			_t = typeof(T);
			List<T> list = new List<T>();
			foreach (FeedbackFeedback feedback in FeedbacksList)
		...
		/// <summary>
		/// Catches FeedbackPlayerEvents and plays this Feedback Player if the channel matches
		/// </summary>
		/// <param name="mmfPlayerEvent"></param>
		public void OnMMEvent(FeedbackPlayerEvent mmfPlayerEvent)
		{
			bool eventMatch = mmfPlayerEvent.ChannelData != null && FeelChannel.Match(mmfPlayerEvent.ChannelData, FeedbackChannelMode, FeedbackChannel, FeedbackChannelDefinition);
			if (!eventMatch)
			{
				return;
			}
			...
			{
				case FeedbackPlayerEvent.Modes.PlayFeedbacks:
					...
					break;
				case FeedbackPlayerEvent.Modes.StopFeedbacks:
					StopFeedbacks();
					break;
				case FeedbackPlayerEvent.Modes.PauseFeedbacks:
					PauseFeedbacks();
					break;
				case FeedbackPlayerEvent.Modes.ResumeFeedbacks:
					ResumeFeedbacks();
					break;
				case FeedbackPlayerEvent.Modes.Initialization:
					Initialization();
					break;
				case FeedbackPlayerEvent.Modes.PlayFeedbacksInReverse:
					PlayFeedbacksInReverse(mmfPlayerEvent.Position, mmfPlayerEvent.FeedbacksIntensity, mmfPlayerEvent.ForceChangeDirection);
					break;
				case FeedbackPlayerEvent.Modes.PlayFeedbacksOnlyIfReversed:
					...
					break;
				case FeedbackPlayerEvent.Modes.PlayFeedbacksOnlyIfNormalDirection:
					...
					break;
				case FeedbackPlayerEvent.Modes.ResetFeedbacks:
					ResetFeedbacks();
					break;
				case FeedbackPlayerEvent.Modes.ChangeDirection:
					ChangeDirection();
					break;
				case FeedbackPlayerEvent.Modes.SetDirectionTopToBottom:
					SetDirection(Directions.TopToBottom);
					break;
				case FeedbackPlayerEvent.Modes.SetDirectionBottomToTop:
					SetDirection(Directions.BottomToTop);
					break;
				case FeedbackPlayerEvent.Modes.RestoreInitialValues:
					RestoreInitialValues();
					break;
				case FeedbackPlayerEvent.Modes.SkipToTheEnd:
					SkipToTheEnd();
					break;
				case FeedbackPlayerEvent.Modes.RefreshCache:
					RefreshCache();
					break;
				default:
		...
		/// <summary>
		/// When we get a SetFeedbackRangeCenterEvent, we set our new range center
		/// </summary>
		...
		protected override void OnDisable()
		{
			...
			if (OnlyPlayIfWithinRange)
			{
				SetFeedbackRangeCenterEvent.Unregister(OnMMSetFeedbackRangeCenterEvent);	
			}
				...
				while ((i >= 0) && (i < FeedbacksList.Count) && (iterations < maxIterationsSafety))
				{
					...
					if ((FeedbacksList[i] != null) && FeedbacksList[i].Active && FeedbacksList[i].ShouldPlayInThisSequenceDirection)
					{
						...
						if (FeedbacksList[i].Pause != null)
						{
							...
							{
								intermediateTotal += ApplyTimeMultiplier((FeedbacksList[i] as FeedbackPause).PauseDuration);
								total += intermediateTotal;
								intermediateTotal = 0f;
							}
							else
							{
								currentPauseDelay += ApplyTimeMultiplier((FeedbacksList[i] as FeedbackPause).PauseDuration);
							}
							...
							if (FeedbacksList[i].LooperPause && ((FeedbacksList[i] as FeedbackLooper).NumberOfLoops > 0))
							{
								...
								{
									lastLoopFoundAt = i;
									loopsLeft = (FeedbacksList[i] as FeedbackLooper).NumberOfLoops - 1;
								}
								
								if ((FeedbacksList[i] as FeedbackLooper).InfiniteLoop)
								{
									_cachedTotalDuration = 999f; 
									return;
								}

								if ((FeedbacksList[i] as FeedbackLooper).LoopAtLastPause)
								{
									i = lastPauseFoundAt;
									total += intermediateTotal;
									intermediateTotal = 0f;
								...
								else if ((FeedbacksList[i] as FeedbackLooper).LoopAtLastLoopStart)
								{
									i = lastLooperStart;
									total += intermediateTotal;
									intermediateTotal = 0f;
		...
		/// <summary>
		/// On Destroy, removes all feedbacks from this FeelFeedbacks to avoid any leftovers
		/// </summary>
		protected override void OnDestroy()
		{
			this.EventStopListening<FeedbackPlayerEvent>();
			IsPlaying = false;
		...
		/// <summary>
		/// Draws gizmos, when the FeedbackPlayer is selected, for all feedbacks that implement the method of the same name 
		/// </summary>
