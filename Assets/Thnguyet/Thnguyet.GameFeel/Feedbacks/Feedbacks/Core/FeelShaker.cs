...
{
	public class FeelShaker : FeelMonoBehaviour
	{
		[InspectorGroup("Shaker Settings", true, 3)]
		...
		public virtual ChannelData ChannelData => new ChannelData(ChannelMode, Channel, ChannelDefinition);
		...
		/// <returns></returns>
		protected virtual bool CheckEventAllowed(ChannelData channelData, bool useRange = false, float range = 0f, Vector3 eventOriginPosition = default(Vector3))
		{
		...
		public virtual float ComputeRangeIntensity(bool useRange, float rangeDistance, bool useRangeFalloff, AnimationCurve rangeFalloff, Vector2 remapRangeFalloff, Vector3 rangePosition)
		{
			...
			float normalizedDistance = FeelMaths.Remap(distanceToCenter, 0f, rangeDistance, 0f, 1f);
			float curveValue = rangeFalloff.Evaluate(normalizedDistance);
			float newIntensity = FeelMaths.Remap(curveValue, 0f, 1f, remapRangeFalloff.x, remapRangeFalloff.y);
			return newIntensity;
		}
	}
}
