...
{
	/// <summary>
	/// A base class, meant to be extended, defining a Feedback. A Feedback is an action triggered by a FeelFeedbacks, usually in reaction to the player's input or actions,
	...
	[System.Serializable]
	[ExecuteAlways]
	public abstract class FeelFeedback : MonoBehaviour
	{
		...
		/// if this is true, this feedback will pause and wait until Resume() is called on its parent FeelFeedbacks to resume execution
		...
		public virtual ChannelData ChannelData(int channel) => _channelData.Set(ChannelModes.Int, channel, null);
		...
		protected FeelFeedbacks _hostMMFeedbacks;
		...
		{
			_hostMMFeedbacks = this.gameObject.GetComponent<FeelFeedbacks>();
		...
		public virtual void Initialization(GameObject owner)
		{
			...
			_hostMMFeedbacks = this.gameObject.GetComponent<FeelFeedbacks>();
			_channelData = new ChannelData(ChannelModes.Int, 0, null);
			...
			get
			{
				switch (Timing.PlayDirection)
				{
					case FeedbackTiming.PlayDirections.FollowMMFeedbacksDirection:
						return (_hostMMFeedbacks.Direction == FeelFeedbacks.Directions.TopToBottom);
			...
			get
			{
				switch (Timing.FeedbacksDirectionCondition)
				{
					...
					case FeedbackTiming.FeedbacksDirectionConditions.OnlyWhenBackwards:
						return (_hostMMFeedbacks.Direction == FeelFeedbacks.Directions.BottomToTop);
				}
				return true;
			}
		}
