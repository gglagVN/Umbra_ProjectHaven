...
{
	/// <summary>
	/// A collection of FeelFeedback, meant to be played altogether.
	...
	/// </summary>
	[AddComponentMenu("")]
	public class FeelFeedbacks : MonoBehaviour
	{
		/// the possible directions FeelFeedbacks can be played
		...
		/// a list of FeelFeedback to trigger
		public List<FeelFeedback> Feedbacks = new List<FeelFeedback>();
		...
		/// Otherwise, you can have this component initialize itself at Awake or Start, and in this case the owner will be the FeelFeedbacks itself
		...
		[Tooltip("the chosen initialization modes. If you use Script, you'll have to initialize manually by calling the " +
		         ...
		         "itself at Awake or Start, and in this case the owner will be the FeelFeedbacks itself")]
		...
		/// whether or not this FeelFeedbacks should invert its direction when all feedbacks have played
		[Tooltip("whether or not this FeelFeedbacks should invert its direction when all feedbacks have played")]
		...
		[FeedbackCondition("ForceTimescaleMode", true)]
		...
		[Condition("RandomizeDuration", true)]
		...
		[FeedbackCondition("UseRangeFalloff", true)]
		...
		[FeedbackVector("Zero","One")]
		public Vector2 RemapRangeFalloff = new Vector2(0f, 1f);
		/// whether or not to ignore SetFeedbackRangeCenterEvent, used to set the RangeCenter from anywhere
		[Tooltip("whether or not to ignore SetFeedbackRangeCenterEvent, used to set the RangeCenter from anywhere")]
		public bool IgnoreRangeEvents = false;

		/// a duration, in seconds, during which triggering a new play of this FeelFeedbacks after it's been played once will be impossible
		[Tooltip("a duration, in seconds, during which triggering a new play of this FeelFeedbacks after it's been played once will be impossible")]
		public float CooldownDuration = 0f;
		/// a duration, in seconds, to delay the start of this FeelFeedbacks' contents play
		[Tooltip("a duration, in seconds, to delay the start of this FeelFeedbacks' contents play")]
		...
		/// a number of UnityEvents that can be triggered at the various stages of this FeelFeedbacks 
		[Tooltip("a number of UnityEvents that can be triggered at the various stages of this FeelFeedbacks")] 
		...
		[HideInInspector]
		/// whether or not this FeelFeedbacks is in debug mode
		public bool DebugActive = false;
		/// whether or not this FeelFeedbacks is playing right now - meaning it hasn't been stopped yet.
		/// if you don't stop your FeelFeedbacks it'll remain true of course
		public bool IsPlaying { get; protected set; }
		/// if this FeelFeedbacks is playing the time since it started playing
		public virtual float ElapsedTime => IsPlaying ? GetTime() - _lastStartAt : 0f;
		/// the amount of times this FeelFeedbacks has been played
		public int TimesPlayed { get; protected set; }

		/// whether or not the execution of this FeelFeedbacks' sequence is being prevented and waiting for a Resume() call
		public bool InScriptDrivenPause;
		/// true if this FeelFeedbacks contains at least one loop
		...
		/// The total duration (in seconds) of all the active feedbacks in this FeelFeedbacks
		public virtual float TotalDuration
		{
			get
			{
				float total = 0f;
				foreach (FeelFeedback feedback in Feedbacks)
				{
		...
		protected virtual void Awake()
		{
			...
			if (AutoPlayOnEnable)
			{
				FeedbacksEnabler enabler = GetComponent<FeedbacksEnabler>(); 
				if (enabler == null)
				{
					enabler = this.gameObject.AddComponent<FeedbacksEnabler>();
				}
		...
		/// <summary>
		/// Initializes the FeelFeedbacks, setting this FeelFeedbacks as the owner
		/// </summary>
		...
		{
			if ((SafeMode == FeelFeedbacks.SafeModes.RuntimeOnly) || (SafeMode == FeelFeedbacks.SafeModes.Full))
			{
				AutoRepair();
			}
		...
		/// <summary>
		/// Changes the player's direction (inverting it - top to bottom becomes bottom to top, top to bottom becomes bottom to top) then plays all feedbacks using the FeelFeedbacks' position as reference, and no attenuation
		/// </summary>
		...
		protected virtual void PlayFeedbacksInternal(Vector3 position, float feedbacksIntensity, bool forceChangeDirection = false)
		{
			...
			// if all FeelFeedbacks are disabled globally, we stop and don't play
		...
		public virtual FeelFeedback AddFeedback(System.Type feedbackType, bool add = true)
		{
			FeelFeedback newFeedback;
            
			#if UNITY_EDITOR
			if (!Application.isPlaying)
			{
				newFeedback = Undo.AddComponent(this.gameObject, feedbackType) as FeelFeedback;
			}
			else
			{
				newFeedback = this.gameObject.AddComponent(feedbackType) as FeelFeedback;
			}
			#else 
                newFeedback = this.gameObject.AddComponent(feedbackType) as FeelFeedback;
			#endif
		...
		/// <returns></returns>
		protected bool FeedbackCanPlay(FeelFeedback feedback)
		{
			if (feedback == null)
			{
				return false;
		...
		public virtual void AutoRepair()
		{
			...
			{
				if (component is FeelFeedback)
				{
					bool found = false;
					for (int i = 0; i < Feedbacks.Count; i++)
					{
						if (Feedbacks[i] == (FeelFeedback)component)
						{
							found = true;
							break;
						}
					...
					if (!found)
					{
						Feedbacks.Add((FeelFeedback)component);
					}
				}
			}
		} 
