...
{
	public class MiniObjectPooler : MonoBehaviour
	{
		...
		protected MiniObjectPool _objectPool;
		protected const int _initialPoolsListCapacity = 5;
        
		static List<MiniObjectPool> _pools = new List<MiniObjectPool>(_initialPoolsListCapacity);

		/// <summary>
		/// Adds a pooler to the static list if needed
		/// </summary>
		/// <param name="pool"></param>
		public static void AddPool(MiniObjectPool pool)
		{
			if (_pools == null)
			{
				_pools = new List<MiniObjectPool>(_initialPoolsListCapacity);    
			}
		...
		/// </summary>
		/// <param name="pool"></param>
		public static void RemovePool(MiniObjectPool pool)
		{
			_pools?.Remove(pool);
		}
		...
		/// <returns></returns>
		public virtual MiniObjectPool ExistingPool(string poolName)
		{
			if (_pools == null)
			{
				_pools = new List<MiniObjectPool>(_initialPoolsListCapacity);    
			...
			if (_pools.Count == 0)
			{
				var pools = FindObjectsByType<MiniObjectPool>(FindObjectsSortMode.None);
				if (pools.Length > 0)
				{
					_pools.AddRange(pools);
				}
			}
			foreach (MiniObjectPool pool in _pools)
			{
		...
		protected virtual void CreateWaitingPool()
		{
			if (!MutualizeWaitingPools)
			{
				...
				_objectPool = this.gameObject.AddComponent<MiniObjectPool>();
				_objectPool.PooledGameObjects = new List<GameObject>();
				return;
			}
			else
			{
				MiniObjectPool waitingPool = ExistingPool(DetermineObjectPoolName(GameObjectToPool));
				...
				else
				{
					...
					_objectPool = newPool.AddComponent<MiniObjectPool>();
					_objectPool.PooledGameObjects = new List<GameObject>();
					AddPool(_objectPool);
				}
			}
		...
		protected virtual GameObject AddOneObjectToThePool()
		{
			...
			GameObject newGameObject = (GameObject)GameObjectExtensions.InstantiateDisabled(GameObjectToPool);
