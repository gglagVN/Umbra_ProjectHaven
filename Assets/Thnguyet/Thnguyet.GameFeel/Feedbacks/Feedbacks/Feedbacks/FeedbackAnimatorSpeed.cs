...
namespace Thnguyet.GameFeel.Feedbacks
{
	...
	public class FeedbackAnimatorSpeed : FeedbackFeedback 
	{
		...
		[FeedbackInspectorGroup("Speed", true, 14, true)]
		...
		[FeedbackEnumCondition("Mode", (int)SpeedModes.InstantThenReset, (int)SpeedModes.OverTime)]
		...
		[FeedbackEnumCondition("Mode", (int)SpeedModes.OverTime)]
		...
		protected virtual IEnumerator ChangeSpeedCo()
		{
			...
			else if (Mode == SpeedModes.OverTime)
			{
				...
				{
					float time = FeedbacksHelpers.Remap(FeedbackTime - _startedAt, 0f, Duration, 0f, 1f);
					float t = Curve.Evaluate(time);
					BoundAnimator.speed = Mathf.Max(0f, FeedbacksHelpers.Remap(t, 0f, 1f, _initialSpeed, newTargetSpeed));
					yield return null;
				}
				BoundAnimator.speed = _initialSpeed;	
				IsPlaying = false;
