...
{
	/// <summary>
	/// This feedback will move the current "head" of an FeedbackPlayer sequence back to another feedback above in the list.
	...
	/// </summary>
	[AddComponentMenu("")]
	[FeedbackHelp("This feedback will move the current 'head' of an FeedbackPlayer sequence back to another feedback above in the list. " +
	...
	public class FeedbackLooper : FeedbackPause
	{
		[FeedbackInspectorGroup("Loop", true, 34)]
        
		[Header("Loop conditions")]
		/// if this is true, this feedback, when met, will cause the FeedbackPlayer to reposition its 'head' to the first pause found above it (going from this feedback to the top), or to the start if none is found
		[Tooltip("if this is true, this feedback, when met, will cause the FeedbackPlayer to reposition its 'head' to the first pause found above it (going from this feedback to the top), or to the start if none is found")]
		public bool LoopAtLastPause = true;
		/// if this is true, this feedback, when met, will cause the FeedbackPlayer to reposition its 'head' to the first LoopStart feedback found above it (going from this feedback to the top), or to the start if none is found
		[Tooltip("if this is true, this feedback, when met, will cause the FeedbackPlayer to reposition its 'head' to the first LoopStart feedback found above it (going from this feedback to the top), or to the start if none is found")]
		...
		[Condition("InfiniteLoop", true, true)]
		...
		/// whether or not to trigger a Loop FeedbacksEvent when this looper is reached
		[Tooltip("whether or not to trigger a Loop FeedbacksEvent when this looper is reached")]
		...
		#if UNITY_EDITOR
		public override Color FeedbackColor { get { return FeedbacksInspectorColors.LooperColor; } }
		public override Color DisplayColor { get { return FeedbacksInspectorColors.LooperColor.Darken(0.25f); } }
		#endif
		...
		public virtual void TriggerOnLoop(FeelFeedbacks source)
		{
			OnLoop.Invoke();

			if (TriggerMMFeedbacksEvents)
			{
				FeedbacksEvent.Trigger(source, FeedbacksEvent.EventTypes.Loop);
			}
		}
