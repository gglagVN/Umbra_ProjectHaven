...
namespace Thnguyet.GameFeel.Feedbacks
{
	...
	public class FeedbackParticlesInstantiation : FeedbackFeedback
	{
		...
		[FeedbackInspectorGroup("Particles Instantiation", true, 37, true)]
		...
		[FeedbackEnumCondition("Mode", (int)Modes.Pool)]
		...
		[FeedbackEnumCondition("Mode", (int)Modes.OnDemand)]
		...
		[FeedbackCondition("ForceStopAction", true)]
		public ParticleSystemStopAction StopAction = ParticleSystemStopAction.None;

		[FeedbackInspectorGroup("Position", true, 29)]
		...
		[FeedbackInspectorGroup("Simulation Speed", true, 43, false)]
		...
		[FeedbackCondition("ForceSimulationSpeed", true)]
		...
		protected MiniObjectPooler _objectPooler; 
		protected List<MiniObjectPooler> _objectPoolers;
		protected GameObject _newGameObject;
		protected bool _poolCreatedOrFound = false;
		protected Vector3 _scriptPosition;
		protected FeelShufflebag<int> _weightShuffleBag;
		...
		protected virtual void CreatePools(FeedbackPlayer owner)
		{
			if (Mode != Modes.Pool)
			{
				return;
			...
			if (!_poolCreatedOrFound)
			{
				...
				_objectPooler = objectPoolGo.AddComponent<MiniObjectPooler>();
				...
				{
					_objectPoolers = new List<MiniObjectPooler>();
					...
					foreach (ParticleSystem ps in RandomParticlePrefabs)
					{
						...
						MiniObjectPooler objectPooler = randomObjectPoolGo.AddComponent<MiniObjectPooler>();
		...
		protected virtual void InitializeWeights()
		{
			...
			_weightShuffleBag = new FeelShufflebag<int>(size);
			if (Mode == Modes.Pool)
			{
			...
			if (Mode == Modes.Pool)
			{
				if (RandomParticlePrefabs.Count == 0)
				{
					if (_objectPooler != null)
					{
						...
						_instantiatedParticleSystem = _newGameObject.FGetComponentNoAlloc<ParticleSystem>();
						if (_instantiatedParticleSystem != null)
						{
							PositionParticleSystem(_instantiatedParticleSystem);
