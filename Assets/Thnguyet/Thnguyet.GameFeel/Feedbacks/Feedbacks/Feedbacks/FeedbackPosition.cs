...
namespace Thnguyet.GameFeel.Feedbacks
{
	...
	public class FeedbackPosition : FeedbackFeedback
	{
		/// a static bool used to disable all feedbacks of this type at once
		public static bool FeedbackTypeAuthorized = true;
		/// sets the inspector color for this feedback
		#if UNITY_EDITOR
		public override Color FeedbackColor => FeedbacksInspectorColors.TransformColor; 
		...
		[FeedbackInspectorGroup("Position Target", true, 61, true)]
		...
		[FeedbackInspectorGroup("Transition", true, 63)]
		...
		[FeedbackEnumCondition("Mode", (int)Modes.AlongCurve)]
		...
		[FeedbackEnumCondition("Mode", (int)Modes.AtoB, (int)Modes.ToDestination)]
		...
		[FeedbackEnumCondition("MovementMode", (int)MovementModes.Duration)]
		...
		[FeedbackEnumCondition("MovementMode", (int)MovementModes.Speed)]
		public float AnimatePositionSpeed = 1f;
		
		/// the FeelTween curve definition to use instead of the animation curve to define the acceleration of the movement
		[Tooltip("the FeelTween curve definition to use instead of the animation curve to define the acceleration of the movement")]
		public TweenType AnimatePositionTween = new TweenType( new AnimationCurve(new Keyframe(0, 0), new Keyframe(1, 1)), "", "Mode", (int)Modes.AtoB, (int)Modes.ToDestination);
		...
		[FeedbackCondition("RandomizeRemap", true)]
		[FeedbackEnumCondition("Mode", (int)Modes.AlongCurve)]
		...
		public TweenType AnimatePositionTweenX = new TweenType(new AnimationCurve(new Keyframe(0, 0), new Keyframe(0.3f, 1f), new Keyframe(0.6f, -1f), new Keyframe(1, 0f)), "AnimateX");
		...
		/// the acceleration of the movement
		[Tooltip("the acceleration of the movement")]
		public TweenType AnimatePositionTweenY = new TweenType(new AnimationCurve(new Keyframe(0, 0), new Keyframe(0.3f, 1f), new Keyframe(0.6f, -1f), new Keyframe(1, 0f)), "AnimateY");
		...
		/// the acceleration of the movement
		[Tooltip("the acceleration of the movement")]
		public TweenType AnimatePositionTweenZ = new TweenType(new AnimationCurve(new Keyframe(0, 0), new Keyframe(0.3f, 1f), new Keyframe(0.6f, -1f), new Keyframe(1, 0f)), "AnimateZ");
		/// if this is true, calling that feedback will trigger it, even if it's in progress. If it's false, it'll prevent any new Play until the current one is over
		[Tooltip("if this is true, calling that feedback will trigger it, even if it's in progress. If it's false, it'll prevent any new Play until the current one is over")] 
		public bool AllowAdditivePlays = false;
		[FeedbackInspectorGroup("Positions", true, 64)]
		...
		[FeedbackEnumCondition("Mode", (int)Modes.AtoB, (int)Modes.AlongCurve)]
		...
		{
			float newValueX = FeelTween.Tween(percent, 0f, 1f, _remapCurveZero * intensityMultiplier, _remapCurveOne * intensityMultiplier, AnimatePositionTweenX);
			float newValueY = FeelTween.Tween(percent, 0f, 1f, _remapCurveZero * intensityMultiplier, _remapCurveOne * intensityMultiplier, AnimatePositionTweenY);
			float newValueZ = FeelTween.Tween(percent, 0f, 1f, _remapCurveZero * intensityMultiplier, _remapCurveOne * intensityMultiplier, AnimatePositionTweenZ);
		...
		protected virtual IEnumerator MoveFromTo(GameObject movingObject, Vector3 pointA, Vector3 pointB, float duration, TweenType tweenType)
		{
			...
			while ((journey >= 0) && (journey <= duration) && (duration > 0))
			{
				float curveValue = FeelTween.Tween(journey, 0f, duration, 0f, 1f, tweenType);
			...
			float finalCurveValue = FeelTween.Tween(finalTime, 0f, 1f, 0f, 1f, tweenType);
		...
		{
			base.OnValidate();
			FeedbacksHelpers.MigrateCurve(AnimatePositionCurve, AnimatePositionTween, Owner);
			FeedbacksHelpers.MigrateCurve(AnimatePositionCurveX, AnimatePositionTweenX, Owner);
			FeedbacksHelpers.MigrateCurve(AnimatePositionCurveY, AnimatePositionTweenY, Owner);
			FeedbacksHelpers.MigrateCurve(AnimatePositionCurveZ, AnimatePositionTweenZ, Owner);
