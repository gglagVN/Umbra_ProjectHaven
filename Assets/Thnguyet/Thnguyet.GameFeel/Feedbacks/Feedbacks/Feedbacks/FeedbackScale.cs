...
namespace Thnguyet.GameFeel.Feedbacks
{
	...
	public class FeedbackScale : FeedbackFeedback
	{
		...
		[FeedbackInspectorGroup("Scale Mode", true, 12, true)]
		...
		[FeedbackInspectorGroup("Scale Animation", true, 13)]
		...
		public TweenType AnimateScaleTweenX = new TweenType( new AnimationCurve(new Keyframe(0, 0), new Keyframe(0.3f, 1.5f), new Keyframe(1, 0)), "AnimateX");
		...
		public TweenType AnimateScaleTweenY = new TweenType( new AnimationCurve(new Keyframe(0, 0), new Keyframe(0.3f, 1.5f), new Keyframe(1, 0)), "AnimateY");
		...
		public TweenType AnimateScaleTweenZ = new TweenType( new AnimationCurve(new Keyframe(0, 0), new Keyframe(0.3f, 1.5f), new Keyframe(1, 0)), "AnimateZ");
		...
		protected virtual IEnumerator ScaleToDestination()
		{
			...
			while ((journey >= 0) && (journey <= duration) && (duration > 0))
			{
				...
				if (AnimateX)
				{
					...
					_newScale.x = FeedbacksHelpers.Remap(_newScale.x, 0f, 1f, RemapCurveZero, RemapCurveOne);
				}

				if (AnimateY)
				{
					_newScale.y = Mathf.LerpUnclamped(_initialScale.y, DestinationScale.y, AnimateScaleTweenY.Evaluate(percent) + Offset);
					_newScale.y = FeedbacksHelpers.Remap(_newScale.y, 0f, 1f, RemapCurveZero, RemapCurveOne);    
				}

				if (AnimateZ)
				{
					_newScale.z = Mathf.LerpUnclamped(_initialScale.z, DestinationScale.z, AnimateScaleTweenZ.Evaluate(percent) + Offset);
					_newScale.z = FeedbacksHelpers.Remap(_newScale.z, 0f, 1f, RemapCurveZero, RemapCurveOne);    
				}
		...
		/// <returns></returns>
		protected virtual IEnumerator AnimateScale(Transform targetTransform, Vector3 vector, float duration, TweenType curveX, TweenType curveY, TweenType curveZ, float remapCurveZero = 0f, float remapCurveOne = 1f)
		{
			...
			while ((journey >= 0) && (journey <= duration) && (duration > 0))
			{
				...
				if (AnimateX)
				{
					...
					vector.x = FeedbacksHelpers.Remap(vector.x, 0f, 1f, remapCurveZero, remapCurveOne);
				...
				if (AnimateY)
				{
					...
					vector.y = FeedbacksHelpers.Remap(vector.y, 0f, 1f, remapCurveZero, remapCurveOne);    
					if (Mode == Modes.Additive)
					{
				...
				if (AnimateZ)
				{
					...
					vector.z = FeedbacksHelpers.Remap(vector.z, 0f, 1f, remapCurveZero, remapCurveOne);    
					if (Mode == Modes.Additive)
					{
		...
		{
			base.OnValidate();
			FeedbacksHelpers.MigrateCurve(AnimateScaleX, AnimateScaleTweenX, Owner);
			FeedbacksHelpers.MigrateCurve(AnimateScaleY, AnimateScaleTweenY, Owner);
			FeedbacksHelpers.MigrateCurve(AnimateScaleZ, AnimateScaleTweenZ, Owner);
