...
namespace Thnguyet.GameFeel.Feedbacks
{
	...
	/// </summary>
	[AddComponentMenu("")]
	[FeedbackHelp("This feedback triggers a TimeScaleEvent, which, if you have a TimeManager object in your scene, will be caught and used to modify the timescale according to the specified settings. These settings are the new timescale (0.5 will be twice slower than normal, 2 twice faster, etc), the duration of the timescale modification, and the optional speed at which to transition between normal and altered time scale.")]
	[MovedFrom(false, null, "Thnguyet.GameFeel.Feedbacks")]
	[System.Serializable]
	[FeedbackPath("Time/Timescale Modifier")]
	public class FeedbackTimescaleModifier : FeedbackFeedback
	{
		...
		[FeedbackInspectorGroup("Timescale Modifier", true, 63)]
		/// the selected mode
		[Tooltip("the selected mode : shake : changes the timescale for a certain duration" +
		         ...
		         "\n- change : sets the timescale to a new value, forever (until you change it again)" +
		         "\n- reset : resets the timescale to its NormalTimescale value, defined in the TimeManager" +
		...
		[FeedbackEnumCondition("Mode", (int)Modes.Shake)]
		...
		[FeedbackInspectorGroup("Interpolation", true, 63)]
		...
		public TimeScaleLerpModes TimescaleLerpMode = TimeScaleLerpModes.Speed;
		/// in Speed mode, the speed at which to lerp the timescale
		[Tooltip("in Speed mode, the speed at which to lerp the timescale")]
		[FeedbackEnumCondition("TimescaleLerpMode", (int)TimeScaleLerpModes.Speed)]
		...
		public TweenType TimescaleLerpCurve = new TweenType( new AnimationCurve(new Keyframe(0, 0), new Keyframe(1, 1)), 
			enumConditionPropertyName:"TimescaleLerpMode", enumConditionValues:(int)TimeScaleLerpModes.Duration); 
		...
		[FeedbackEnumCondition("TimescaleLerpMode", (int)TimeScaleLerpModes.Duration)]
		...
		public TweenType TimescaleLerpCurveOnUnfreeze = new TweenType( new AnimationCurve(new Keyframe(0, 0), new Keyframe(1, 1)), 
			enumConditionPropertyName:"TimescaleLerpMode", enumConditionValues:(int)TimeScaleLerpModes.Duration);
		...
		public override float FeedbackDuration {
			get
			{
				...
				if (TimescaleLerpMode == TimeScaleLerpModes.Duration)
				{
		...
		protected override void CustomPlayFeedback(Vector3 position, float feedbacksIntensity = 1.0f)
		{
			...
			{
				case Modes.Shake:
					TimeScaleEvent.Trigger(TimeScaleMethods.For, TimeScale, TimeScaleDuration, TimeScaleLerp, TimeScaleLerpSpeed, false, TimescaleLerpMode, TimescaleLerpCurve, TimescaleLerpDuration, TimeScaleLerpOnUnfreeze, TimescaleLerpCurveOnUnfreeze, TimescaleLerpDurationOnUnfreeze);
					break;
				case Modes.Change:
					TimeScaleEvent.Trigger(TimeScaleMethods.For, TimeScale, 0f, TimeScaleLerp, TimeScaleLerpSpeed, true, TimescaleLerpMode, TimescaleLerpCurve, TimescaleLerpDuration, TimeScaleLerpOnUnfreeze, TimescaleLerpCurveOnUnfreeze, TimescaleLerpDurationOnUnfreeze);
					break;
				case Modes.Reset:
					TimeScaleEvent.Trigger(TimeScaleMethods.Reset, TimeScale, 0f, false, 0f, true);
					break;
				case Modes.Unfreeze:
					TimeScaleEvent.Trigger(TimeScaleMethods.Unfreeze, TimeScale, 0f, false, 0f, true);
					break;
			}     
		}
		...
		public override void OnValidate()
		{
			...
			if (TimescaleLerpCurve.EnumConditions[(int)TimeScaleLerpModes.Duration] == false)
			{
				TimescaleLerpCurve.EnumConditions[(int)TimeScaleLerpModes.Duration] = true;
				TimescaleLerpCurveOnUnfreeze.EnumConditions[(int)TimeScaleLerpModes.Duration] = true;
			}
		}
	}
}