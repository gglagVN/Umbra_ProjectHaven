...
namespace Thnguyet.GameFeel.Feedbacks
{
	...
	/// This is designed to be spawned by a FloatingTextSpawner, not used on its own.
	/// It also requires a specific hierarchy. You'll find examples of it in the Tools/Tools/FeelFloatingText/Prefabs folder
	/// </summary>
	public class FeelFloatingText : MonoBehaviour
	{
		[Header("Bindings")]
		...
		/// the FollowTarget component associated to this floating text
		...
		protected FloatingTextSpawner.AlignmentModes _alignmentMode;
		...
		protected virtual void Initialization()
		{
			...
			FollowTarget = this.gameObject.GetOrAddComponent<FollowTarget>();
		}
		...
		protected virtual void UpdateFloatingText()
		{
			...
			_remappedTime = FeelMaths.Remap(_elapsedTime, 0f, _lifetime, 0f, 1f);
		...
		protected virtual void HandleMovement()
		{
			...
			if (_animateMovement)
			{
				...
				_newPosition.x = _animateX ? FeelMaths.Remap(_animateXCurve.Evaluate(_remappedTime), 0f, 1, _remapXZero, _remapXOne) : 0f;
				_newPosition.y = _animateY ? FeelMaths.Remap(_animateYCurve.Evaluate(_remappedTime), 0f, 1, _remapYZero, _remapYOne) : 0f;
				_newPosition.z = _animateZ ? FeelMaths.Remap(_animateZCurve.Evaluate(_remappedTime), 0f, 1, _remapZZero, _remapZOne) : 0f;
		...
		protected virtual void HandleOpacity()
		{
			if (_animateOpacity)
			{
				float newOpacity = FeelMaths.Remap(_animateOpacityCurve.Evaluate(_remappedTime), 0f, 1f, _remapOpacityZero, _remapOpacityOne);
		...
		protected virtual void HandleScale()
		{
			if (_animateScale)
			{
				_newScale = Vector3.one * FeelMaths.Remap(_animateScaleCurve.Evaluate(_remappedTime), 0f, 1f, _remapScaleZero, _remapScaleOne);
		...
		{
			if (_alignmentMode == FloatingTextSpawner.AlignmentModes.Fixed)
			{
				MovingPart.transform.up = _fixedAlignment;
			}
			else if (_alignmentMode == FloatingTextSpawner.AlignmentModes.MatchInitialDirection)
			{
				MovingPart.transform.up = this.transform.up;
			}
			else if (_alignmentMode == FloatingTextSpawner.AlignmentModes.MatchMovementDirection)
			{
		...
		public virtual void SetProperties(string value, float lifetime, Vector3 direction, bool animateMovement, 
			FloatingTextSpawner.AlignmentModes alignmentMode, Vector3 fixedAlignment,
