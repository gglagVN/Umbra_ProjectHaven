...
namespace Thnguyet.GameFeel.Feedbacks
{
	...
	public enum SequenceTrackStates { Idle, Down, Up }
	...
	public class SequenceNote
	{
		...
		public virtual SequenceNote Copy()
		{
			SequenceNote newNote = new SequenceNote();
			newNote.ID = this.ID;
	...
	public class SequenceTrack
	{
		...
		[FeedbackReadOnly]
		public SequenceTrackStates State = SequenceTrackStates.Idle;
		[HideInInspector]
		...
		public virtual void SetDefaults(int index)
		{
			...
			{
				ID = index;
				TrackColor = FeelSequence.RandomSequenceColor();
				Key = KeyCode.Space;
				Active = true;
	...
	{
		public List<SequenceNote> Line;
	}

	/// <summary>
	/// This scriptable object holds "sequences", data used to record and play events in sequence
	/// Sequences can be played by FeelFeedbacks from their Timing section, by Sequencers and potentially other classes
	/// </summary>
	[CreateAssetMenu(menuName = "Thnguyet/GameFeel/Sequencer/FeelSequence")]
	public class FeelSequence : ScriptableObject
	{
		[Header("Sequence")]
		...
		public SequenceList OriginalSequence;
		...
		public List<SequenceTrack> SequenceTracks;
		...
		public List<SequenceList> QuantizedSequence;
        
		[Space]
		[Header("Controls")]
		[FeedbackInspectorButton("RandomizeTrackColors")]
		public bool RandomizeTrackColorsButton;
        
		protected float[] _quantizedBeats; 
		protected List<SequenceNote> _deleteList;
		...
		/// <param name="p2"></param>
		/// <returns></returns>
		static int SortByTimestamp(SequenceNote p1, SequenceNote p2)
		{
		...
		/// </summary>
		public virtual void QuantizeSequenceToBPM(List<SequenceNote> baseSequence)
		{
			float sequenceLength = Length;
			float beatDuration = 60f / TargetBPM;
			int numberOfBeatsInSequence = (int)(sequenceLength / beatDuration);
			QuantizedSequence = new List<SequenceList>();
			_deleteList = new List<SequenceNote>();
			_deleteList.Clear();
			...
			{
				QuantizedSequence.Add(new SequenceList());
				QuantizedSequence[i].Line = new List<SequenceNote>();
				for (int j = 0; j < numberOfBeatsInSequence; j++)
				{
					...
					foreach (SequenceNote note in baseSequence)
					{
		...
		{
			foreach(SequenceTrack track in SequenceTracks)
			{
