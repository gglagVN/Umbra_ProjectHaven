...
{
	/// <summary>
	/// This class lets you design a quantized FeelSequence using a sequencer interface, and lets you play a quantized sequence, triggering events on beats if specified
	/// </summary>
	[AddComponentMenu("Thnguyet/GameFeel/Feedbacks/Sequencing/Sequencer")]
	public class FeelSequencer : MonoBehaviour
	{
		...
		[HideInInspector]
		public FeelSequence LastSequence;
		[HideInInspector]
		...
		public virtual void ApplyToggleActive(int trackIndex)
		{
		...
		public virtual void ApplySequencerLengthToSequence()
		{
			...
			{
				Sequence.QuantizedSequence = new List<SequenceList>();

				for (int trackIndex = 0; trackIndex < Sequence.SequenceTracks.Count; trackIndex++)
				{
					Sequence.QuantizedSequence.Add(new SequenceList());
					Sequence.QuantizedSequence[trackIndex].Line = new List<SequenceNote>();
					for (int i = 0; i < SequencerLength; i++)
					{
						SequenceNote note = new SequenceNote();
						note.ID = -1;
