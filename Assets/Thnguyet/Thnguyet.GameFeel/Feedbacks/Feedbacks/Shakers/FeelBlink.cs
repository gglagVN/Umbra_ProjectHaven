...
namespace Thnguyet.GameFeel.Feedbacks
{
	...
	/// </summary>
	[AddComponentMenu("Thnguyet/GameFeel/Feedbacks/Shakers/Various/Blink")]
	public class FeelBlink : FeelMonoBehaviour
	{
		...
		[InspectorGroup("Blink Method", true, 17)] 
		...
		[FeedbackEnumCondition("Method", (int)Methods.SetGameObjectActive)]
		public GameObject TargetGameObject;
		/// the target renderer to work with
		[Tooltip("the target renderer to work with")]
		[FeedbackEnumCondition("Method", (int)Methods.MaterialAlpha, (int)Methods.MaterialEmissionIntensity, (int)Methods.ShaderFloatValue)]
		public Renderer TargetRenderer;
		...
		[InspectorGroup("Extra Targets", true, 12)] 
		...
		[FeedbackCondition("ForceStateOnExit", true)]
		public States StateOnExit = States.On;

		[InspectorGroup("TimeScale", true, 120)] 
		/// whether or not this FeelBlink should operate on unscaled time 
		[Tooltip("whether or not this FeelBlink should operate on unscaled time")]
		public TimescaleModes TimescaleMode = TimescaleModes.Scaled;
        
		[InspectorGroup("Sequence", true, 121)] 
		...
		[InspectorGroup("Debug", true, 122)] 
		
		[InspectorButtonBar(new string[] { "ToggleBlinking", "StartBlinking", "StopBlinking" }, 
		...
		/// </summary>
		protected virtual void ApplyBlink()
		{
			...
			{
				float t = FeedbacksHelpers.Remap(GetTime() - _lastBlinkAt, 0f, _currentLerpDuration, 0f, 1f);
				newValue = Curve.Evaluate(t);
				newValue = FeedbacksHelpers.Remap(newValue, 0f, 1f, initialValue, targetValue);
			}
			else
			{
		...
		protected virtual void InitializeBlinkProperties()
		{
			if (Phases.Count == 0)
			{
				Debug.LogError("FeelBlink : You need to define at least one phase for this component to work.");
				this.enabled = false;
				return;
			}
