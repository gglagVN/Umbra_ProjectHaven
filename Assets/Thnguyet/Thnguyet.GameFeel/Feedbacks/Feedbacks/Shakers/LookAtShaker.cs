...
{
	/// <summary>
	/// Add this to an object and it'll be able to listen for FLookAtShakeEvents, and when one is received, it will rotate its associated transform accordingly
	/// </summary>
	public class LookAtShaker : FeelShaker
	{
		[InspectorGroup("Look at settings", true, 37)]
		...
		public TweenType LookAtTween = new TweenType( new AnimationCurve(new Keyframe(0, 0), new Keyframe(1, 1)));
		...
		[InspectorGroup("What we want to rotate", true, 37)]
		...
		public FeedbackLookAt.UpwardVectors UpwardVector = FeedbackLookAt.UpwardVectors.Up;

		[InspectorGroup("What we want to look at", true, 37)]
		...
		public FeedbackLookAt.LookAtTargetModes LookAtTargetMode = FeedbackLookAt.LookAtTargetModes.Transform;
		/// the transform we want to look at 
		[Tooltip("the transform we want to look at")]
		[FeedbackEnumCondition("LookAtTargetMode", (int)FeedbackLookAt.LookAtTargetModes.Transform)]
		...
		[FeedbackEnumCondition("LookAtTargetMode", (int)FeedbackLookAt.LookAtTargetModes.TargetWorldPosition)]
		...
		[FeedbackEnumCondition("LookAtTargetMode", (int)FeedbackLookAt.LookAtTargetModes.Direction)]
		public Vector3 LookAtDirection = Vector3.forward;
		
		[InspectorGroup("Test", true, 46)]
		...
		/// </summary>
		public struct LookAtShakeEvent
		{
			...
			public delegate void Delegate(float duration, 
				bool lockXAxis, bool lockYAxis, bool lockZAxis, FeedbackLookAt.UpwardVectors upwardVector, FeedbackLookAt.LookAtTargetModes lookAtTargetMode,Transform lookAtTarget, Vector3 lookAtTargetWorldPosition, Vector3 lookAtDirection, Transform transformToRotate, TweenType lookAtTween,
				...
				float feedbacksIntensity = 1.0f, ChannelData channelData = null, bool resetShakerValuesAfterShake = true, 
		...
		protected TweenType _originalLookAtTween;
		protected bool _originalLockXAxis;
		protected bool _originalLockYAxis;
		protected bool _originalLockZAxis;
		protected FeedbackLookAt.UpwardVectors _originalUpwardVector;
		protected FeedbackLookAt.LookAtTargetModes _originalLookAtTargetMode;
		...
		protected virtual void ApplyRotation(float journey)
		{
			...
			{
				case FeedbackLookAt.LookAtTargetModes.Transform:
					_lookAtPosition = LookAtTarget.position;
					break;
				case FeedbackLookAt.LookAtTargetModes.TargetWorldPosition:
					_lookAtPosition = LookAtTargetWorldPosition;
					break;
				case FeedbackLookAt.LookAtTargetModes.Direction:
		...
		{
			base.StartListening();
			LookAtShakeEvent.Register(OnMMLookAtShakeEvent);
		}
		...
		{
			base.StopListening();
			LookAtShakeEvent.Unregister(OnMMLookAtShakeEvent);
		}
	}
}
