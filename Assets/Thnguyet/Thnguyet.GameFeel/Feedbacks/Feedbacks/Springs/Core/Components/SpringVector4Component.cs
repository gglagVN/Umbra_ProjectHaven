...
{
	/// <summary>
	/// An event used to pilot a SpringVector4 component
	/// </summary>
	public struct SpringVector4Event
	{
		static SpringVector4Event e;
		...
		public static void Trigger(SpringCommands command, SpringComponentBase targetSpring, ChannelData channelData, 
	...
	/// </summary>
	public abstract class SpringVector4<T> : SpringComponentBase, EventListener<SpringVector4Event> where T:Component
	{
		[InspectorGroup("Target", true, 17)] 
		public T Target;
		
		[InspectorGroup("Channel & TimeScale", true, 16, true)] 
		...
		[InspectorGroup("Spring Settings", true, 18)]
		[Header("SpringVector4")]
		public SpringVector4 Spring = new SpringVector4();
		
		[InspectorGroup("Randomness", true, 12, true)]
		...
		[InspectorGroup("Test", true, 20, true)]
		...
		[InspectorButtonBar(new string[] { "MoveTo", "MoveToAdditive", "MoveToSubtractive", "MoveToRandom", "MoveToInstant" }, 
		...
		[InspectorButtonBar(new string[] { "Bump", "BumpRandom" }, 
			new string[] { "TestBump", "TestBumpRandom" }, 
			new bool[] { true, true },
			new string[] { "main-call-to-action", "" })]
		public bool BumpToToolbar;
		
		[InspectorButtonBar(new string[] { "Stop", "Finish", "RestoreInitialValue", "ResetInitialValue" }, 
			new string[] { "Stop", "Finish", "RestoreInitialValue", "ResetInitialValue" }, 
			new bool[] { true, true, true, true },
			new string[] { "", "", "", "" })]
		public bool OtherControlsToToolbar;
		
		public override bool LowVelocity => (Mathf.Abs(Spring.SpringX.Velocity) + Mathf.Abs(Spring.SpringY.Velocity) + Mathf.Abs(Spring.SpringZ.Velocity) + Mathf.Abs(Spring.SpringW.Velocity)) < _velocityLowThreshold;
		...
		{
			Activate();
			Spring.BumpRandom(BumpAmountRandomValueMin, BumpAmountRandomValueMax);
		}

		public virtual void BumpRandom(Vector4 min, Vector4 max)
		{
			Activate();
			Spring.BumpRandom(min, max);
		}
		...
		{
			Spring.RestoreInitialValue();
			ApplyValue(Spring.CurrentValue);
		}
		...
		{
			Spring.UpdateSpringValue(DeltaTime);
			ApplyValue(Spring.CurrentValue);
		}
		...
		protected override void Initialization()
		{
			base.Initialization();
			GrabCurrentValue();
			Spring.SetInitialValue(Spring.CurrentValue);
			Spring.TargetValue = Spring.CurrentValue;
		}
		...
		protected override void GrabCurrentValue()
		{
			...
			Spring.CurrentValue = TargetVector4;
		}
		...
		public void OnMMEvent(SpringVector4Event springEvent)
		{
			bool eventMatch = springEvent.ChannelData != null && FeelChannel.Match(springEvent.ChannelData, ChannelMode, Channel, ChannelDefinition);
			...
			{
				Spring.SetDamping(springEvent.NewDamping);
			}
			if (springEvent.OverrideFrequency)
			{
				Spring.SetFrequency(springEvent.NewFrequency);
			}
		...
		protected override void Awake()
		{
			...
			base.Awake();
			this.EventStartListening<SpringVector4Event>();
		}

		protected void OnDestroy()
		{
			this.EventStopListening<SpringVector4Event>();
		}
