...
namespace Thnguyet.GameFeel.ThirdParty
{
	...
	/// with Color Adjustments active, and a ColorAdjustmentsShaker_HDRP component.
	/// </summary>
	[AddComponentMenu("")]
	[System.Serializable]
	#if GAMEFEEL_HDRP
	...
	[FeedbackHelp("This feedback allows you to control color adjustments' post exposure, hue shift, saturation and contrast over time. " +
	              "It requires you have in your scene an object with a Volume " +
	              "with Color Adjustments active, and a ColorAdjustmentsShaker_HDRP component.")]
	public class FeedbackColorAdjustments_HDRP : FeedbackFeedback
	{
		...
		[FeedbackInspectorGroup("Color Grading", true, 16)]
		...
		[FeedbackInspectorGroup("Post Exposure", true, 15)]
		...
		[FeedbackInspectorGroup("Hue Shift", true, 14)]
		...
		[FeedbackInspectorGroup("Saturation", true, 13)]
		...
		[FeedbackInspectorGroup("Contrast", true, 12)]
		/// the curve used to animate the focal length value on
		[Tooltip("the curve used to animate the focal length value on")]
		...
		[Tooltip("the selected color filter mode :" +
		         ...
		         "gradient : evaluates the color over time on that gradient, from left to right," +
		         "interpolate : lerps from the current color to the destination one ")]
		public ColorAdjustmentsShaker_HDRP.ColorFilterModes ColorFilterMode = ColorAdjustmentsShaker_HDRP.ColorFilterModes.None;
		...
		[FeedbackEnumCondition("ColorFilterMode", (int)ColorAdjustmentsShaker_HDRP.ColorFilterModes.Gradient)]
		[GradientUsage(true)]
		...
		[FeedbackEnumCondition("ColorFilterMode", (int) ColorAdjustmentsShaker_HDRP.ColorFilterModes.Interpolate)]
		...
		protected override void CustomPlayFeedback(Vector3 position, float feedbacksIntensity = 1.0f)
		{
			...
			ColorAdjustmentsShakeEvent_HDRP.Trigger(ShakePostExposure, RemapPostExposureZero, RemapPostExposureOne,
		...
		public override void AutomaticShakerSetup()
		{
			...
			HDRPHelpers.GetOrCreateVolume<ColorAdjustments, ColorAdjustmentsShaker_HDRP>(Owner, "ColorAdjustments");
			#endif
		}
	}
}