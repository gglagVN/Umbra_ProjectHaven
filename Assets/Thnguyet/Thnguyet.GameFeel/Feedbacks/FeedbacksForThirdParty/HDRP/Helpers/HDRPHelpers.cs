...
{
	public class HDRPHelpers : MonoBehaviour
	{
		#if UNITY_EDITOR && GAMEFEEL_HDRP
		public static void GetOrCreateVolume<T, U>(FeedbackPlayer owner, string feedbackName) where T:VolumeComponent where U:FeelShaker
		{
			...
			if (volume == null)
			{
				GameObject postProcessingObject = GameObject.Instantiate(Resources.Load<GameObject>("DefaultHDRPVolume"));
