...
namespace Thnguyet.GameFeel.ThirdParty
{
	...
	#endif
	[AddComponentMenu("Thnguyet/GameFeel/Feedbacks/Shakers/PostProcessing/Color Adjustments Shaker HDRP")]
	public class ColorAdjustmentsShaker_HDRP : FeelShaker
	{
		/// whether or not to add to the initial value
		public bool RelativeValues = true;

		[InspectorGroup("Post Exposure", true, 44)]
		...
		[InspectorGroup("Hue Shift", true, 45)]
		...
		[InspectorGroup("Saturation", true, 46)]
		...
		[InspectorGroup("Contrast", true, 47)]
		/// the curve used to animate the focal length value on
		[Tooltip("the curve used to animate the focal length value on")]
		...
		[InspectorGroup("Color Filter", true, 48)]
		...
		[FeedbackEnumCondition("ColorFilterMode", (int)ColorFilterModes.Gradient)]
		[GradientUsage(true)]
		...
		[FeedbackEnumCondition("ColorFilterMode", (int) ColorFilterModes.Interpolate)]
		...
		protected override void Shake()
		{
			...
			_remappedTimeSinceStart = FeedbacksHelpers.Remap(Time.time - _shakeStartedTimestamp, 0f, ShakeDuration, 0f, 1f);
		...
		public virtual void OnMMColorGradingShakeEvent(AnimationCurve shakePostExposure, float remapPostExposureZero, float remapPostExposureOne,
			...
			float attenuation = 1.0f, ChannelData channelData = null, bool resetShakerValuesAfterShake = true, bool resetTargetValuesAfterShake = true, 
		...
		{
			base.StartListening();
			ColorAdjustmentsShakeEvent_HDRP.Register(OnMMColorGradingShakeEvent);
		}
		...
		{
			base.StopListening();
			ColorAdjustmentsShakeEvent_HDRP.Unregister(OnMMColorGradingShakeEvent);
		}
		#endif
	}
	...
	/// </summary>
	public struct ColorAdjustmentsShakeEvent_HDRP
	{
		...
		public delegate void Delegate(AnimationCurve shakePostExposure, float remapPostExposureZero, float remapPostExposureOne,
			...
			AnimationCurve shakeSaturation, float remapSaturationZero, float remapSaturationOne,
			AnimationCurve shakeContrast, float remapContrastZero, float remapContrastOne,
			ColorAdjustmentsShaker_HDRP.ColorFilterModes colorFilterMode, Gradient colorFilterGradient, Color colorFilterDestination,AnimationCurve colorFilterCurve,  
			float duration, bool relativeValues = false,
			float attenuation = 1.0f, ChannelData channelData = null, bool resetShakerValuesAfterShake = true, bool resetTargetValuesAfterShake = true, 
			bool forwardDirection = true, TimescaleModes timescaleMode = TimescaleModes.Scaled, bool stop = false, bool restore = false);
