...
namespace Thnguyet.GameFeel.ThirdParty
{
	...
	/// </summary>
	[AddComponentMenu("Thnguyet/GameFeel/Feedbacks/Shakers/PostProcessing/Bloom Shaker")]
	#if GAMEFEEL_POSTPROCESSING
	[RequireComponent(typeof(PostProcessVolume))]
	#endif
	public class BloomShaker : FeelShaker
	{
		/// whether or not to add to the initial value
		public bool RelativeValues = true;

		[InspectorGroup("Bloom Intensity", true, 45)]
		...
		[InspectorGroup("Bloom Threshold", true, 46)]
		...
		public virtual void OnBloomShakeEvent(AnimationCurve intensity, float duration, float remapMin, float remapMax,
			...
			float feedbacksIntensity = 1.0f, ChannelData channelData = null, bool resetShakerValuesAfterShake = true, bool resetTargetValuesAfterShake = true, bool forwardDirection = true, TimescaleModes timescaleMode = TimescaleModes.Scaled, bool stop = false, bool restore = false)
		{
		...
		{
			base.StartListening();
			BloomShakeEvent.Register(OnBloomShakeEvent);
		}
		...
		{
			base.StopListening();
			BloomShakeEvent.Unregister(OnBloomShakeEvent);
		}
		#endif
	}
