...
namespace Thnguyet.GameFeel.Feedbacks
{
	...
	public class FeedbackTMPAlpha : FeedbackFeedback
	{
		...
		[FeedbackInspectorGroup("Alpha", true, 16)]
		...
		[FeedbackEnumCondition("AlphaMode", (int)AlphaModes.Interpolate, (int)AlphaModes.ToDestination)]
		public float Duration = 0.2f;
		/// the alpha to apply when in instant mode
		[Tooltip("the alpha to apply when in instant mode")]
		[FeedbackEnumCondition("AlphaMode", (int)AlphaModes.Instant)]
		...
		public TweenType Curve = new TweenType(FeelTween.TweenCurve.EaseInCubic, "", "AlphaMode", (int)AlphaModes.Interpolate, (int)AlphaModes.ToDestination);
		/// the value to which the curve's 0 should be remapped
		[Tooltip("the value to which the curve's 0 should be remapped")]
		[FeedbackEnumCondition("AlphaMode", (int)AlphaModes.Interpolate)]
		...
		[FeedbackEnumCondition("AlphaMode", (int)AlphaModes.ToDestination)]
		...
		protected virtual void SetAlpha(float time)
		{
			...
			{
				newAlpha = FeelTween.Tween(time, 0f, 1f, CurveRemapZero, CurveRemapOne, Curve);    
			}
			else if (AlphaMode == AlphaModes.ToDestination)
			{
				newAlpha = FeelTween.Tween(time, 0f, 1f, _initialAlpha, DestinationAlpha, Curve);
			}
