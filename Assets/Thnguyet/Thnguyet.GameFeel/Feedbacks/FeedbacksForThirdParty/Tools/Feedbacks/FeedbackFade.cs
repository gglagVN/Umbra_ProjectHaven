...
namespace Thnguyet.GameFeel.Feedbacks
{
	...
	public class FeedbackFade : FeedbackFeedback
	{
		...
		[FeedbackInspectorGroup("Fade", true, 43)]
		...
		[FeedbackEnumCondition("PositionMode", (int)PositionModes.Transform)]
		public Transform TargetTransform;
		/// the coordinates on which to center the fadet
		[Tooltip("the coordinates on which to center the fade")]
		[FeedbackEnumCondition("PositionMode", (int)PositionModes.WorldPosition)]
		...
		/// this field lets you bind a specific Fader to this feedback. If left empty, the feedback will trigger a FadeEvent instead, targeting all matching faders. If you fill it, only that specific fader will be targeted.
		[Tooltip("this field lets you bind a specific Fader to this feedback. If left empty, the feedback will trigger a FadeEvent instead, targeting all matching faders. If you fill it, only that specific fader will be targeted.")]
		public Fader TargetFader;
		...
		protected override void CustomPlayFeedback(Vector3 position, float feedbacksIntensity = 1.0f)
		{
			...
			else
			{
				...
				{
					case FadeTypes.Custom:
						FadeEvent.Trigger(FeedbackDuration, TargetAlpha, Curve, ID, IgnoreTimeScale, _position);
						break;
					case FadeTypes.FadeIn:
						FadeInEvent.Trigger(FeedbackDuration, Curve, ID, IgnoreTimeScale, _position);
						break;
					case FadeTypes.FadeOut:
						FadeOutEvent.Trigger(FeedbackDuration, Curve, ID, IgnoreTimeScale, _position);
						break;
				}
			}
		}
		...
		protected override void CustomRestoreInitialValues()
		{
			...
			FadeStopEvent.Trigger(ID, true);
		}
		
		/// <summary>
		/// Automatically tries to add a Fader setup to the scene
		/// </summary>
		public override void AutomaticShakerSetup()
		{
			if (Object.FindAnyObjectByType<Fader>() != null)
			{
				return;
			}
			
			(Canvas canvas, bool createdNewCanvas) = Owner.gameObject.FindOrCreateObjectOfType<Canvas>("FadeCanvas", null);
			canvas.renderMode = RenderMode.ScreenSpaceOverlay;
			(Image image, bool createdNewImage) = canvas.gameObject.FindOrCreateObjectOfType<Image>("FadeImage", canvas.transform, true);
