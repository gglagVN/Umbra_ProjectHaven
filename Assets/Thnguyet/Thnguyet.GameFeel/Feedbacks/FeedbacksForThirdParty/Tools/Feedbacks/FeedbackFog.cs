...
namespace Thnguyet.GameFeel.Feedbacks
{
	...
	public class FeedbackFog : FeedbackFeedback
	{
		...
		[FeedbackInspectorGroup("Fog", true, 24)]
		...
		[FeedbackEnumCondition("Mode", (int)Modes.OverTime)]
		public float Duration = 2f;
		/// if this is true, calling that feedback will trigger it, even if it's in progress. If it's false, it'll prevent any new Play until the current one is over
		[Tooltip("if this is true, calling that feedback will trigger it, even if it's in progress. If it's false, it'll prevent any new Play until the current one is over")] 
		public bool AllowAdditivePlays = false;

		[FeedbackInspectorGroup("Fog Density", true, 25)]
		...
		public TweenType DensityCurve = new TweenType(new AnimationCurve(new Keyframe(0, 0), new Keyframe(0.3f, 1f), new Keyframe(1, 0)));
		...
		[FeedbackInspectorGroup("Fog Start Distance", true, 26)]
		...
		public TweenType StartDistanceCurve = new TweenType(new AnimationCurve(new Keyframe(0, 0), new Keyframe(0.3f, 1f), new Keyframe(1, 0)));
		...
		[FeedbackInspectorGroup("Fog End Distance", true, 27)]
		...
		public TweenType EndDistanceCurve = new TweenType(new AnimationCurve(new Keyframe(0, 0), new Keyframe(0.3f, 1f), new Keyframe(1, 0)));
		...
		[FeedbackInspectorGroup("Fog Color", true, 28)]
		...
		protected virtual void SetFogValues(float time, float intensityMultiplier)
		{
			...
			if (ModifyFogDensity)
			{
				RenderSettings.fogDensity = FeelTween.Tween(time, 0f, 1f, DensityRemapZero, DensityRemapOne, DensityCurve) * intensityMultiplier;
			}

			if (ModifyStartDistance)
			{
				RenderSettings.fogStartDistance = FeelTween.Tween(time, 0f, 1f, StartDistanceRemapZero, StartDistanceRemapOne, StartDistanceCurve);
			}

			if (ModifyEndDistance)
			{
				RenderSettings.fogEndDistance = FeelTween.Tween(time, 0f, 1f, EndDistanceRemapZero, EndDistanceRemapOne, EndDistanceCurve);
			}
		}
