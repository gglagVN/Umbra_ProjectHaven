...
namespace Thnguyet.GameFeel.Feedbacks
{
	...
	public class FeedbackLineRenderer : FeedbackFeedback
	{
		...
		[FeedbackInspectorGroup("Line Renderer", true, 24, true)]
		...
		protected virtual void SetLineRendererValues(float time, float intensityMultiplier)
		{
			if (ModifyColor)
			{
				...
				{
					TargetLineRenderer.colorGradient = FeelColors.LerpGradients(_initialColor, NewColor, time);	
				}
				else
				{
					TargetLineRenderer.colorGradient = FeelColors.LerpGradients(NewColor, _firstColor, time);
				}
			}
			...
			if (ModifyWidth)
			{
				if (NormalPlayDirection)
				{
					TargetLineRenderer.widthCurve = AnimationCurves.LerpAnimationCurves(_initialWidth, NewWidth, time);	
				}
				else
				{
					TargetLineRenderer.widthCurve = AnimationCurves.LerpAnimationCurves(NewWidth, _firstWidth, time);
				}
			}
		}
