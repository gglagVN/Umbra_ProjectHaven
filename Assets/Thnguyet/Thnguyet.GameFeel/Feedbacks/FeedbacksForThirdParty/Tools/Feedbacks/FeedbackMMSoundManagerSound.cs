...
{    
	/// <summary>
	/// This feedback will let you play a sound via the FeelSoundManager. You will need a game object in your scene with a FeelSoundManager object on it for this to work.
	/// </summary>
	[ExecuteAlways]
	[AddComponentMenu("")]
	[MovedFrom(false, null, "Thnguyet.GameFeel.Feedbacks.Tools")]
	[System.Serializable]
	[FeedbackPath("Audio/FeelSoundManager Sound")]
	[FeedbackHelp("This feedback will let you play a sound via the FeelSoundManager. You will need a game object in your scene with a FeelSoundManager object on it for this to work.")]
	public class FeedbackMMSoundManagerSound : FeedbackFeedback
	{
		...
		[FeedbackInspectorGroup("Sound", true, 14, true)]
		...
		[FeedbackEnumCondition("Source", (int)Sources.AudioClip)]
		...
		[FeedbackEnumCondition("Source", (int)Sources.AudioResource)]
		public AudioResource AudioResourceToPlay;

		[FeedbackInspectorGroup("Random Sound", true, 39, true)]
		...
		[FeedbackCondition("SequentialOrder", true)]
		...
		[FeedbackCondition("SequentialOrderHoldLast", true)]
		...
		[FeedbackInspectorGroup("Scriptable Object", true, 14, true)]
		/// a scriptable object (created via the Create/Thnguyet/GameFeel/Audio/FeedbackSoundData menu) to define settings that will override all other settings on this feedback
		[Tooltip("a scriptable object (created via the Create/Thnguyet/GameFeel/Audio/FeedbackSoundData menu) to define settings that will override all other settings on this feedback")]
		public FeedbackMMSoundManagerSoundData SoundDataSO;

		[FeedbackInspectorGroup("Sound Properties", true, 24)]
		[Header("Volume")]
		...
		[FeedbackInspectorGroup("SoundManager Options", true, 28)]
		...
		public FeelSoundManager.SoundManagerTracks MmSoundManagerTrack = FeelSoundManager.SoundManagerTracks.Sfx;
		...
		[FeedbackInspectorGroup("Fade In", true, 30)]
		...
		public TweenType FadeInTween = new TweenType(FeelTween.TweenCurve.EaseInOutQuartic, "FadeIn");
		
		[FeedbackInspectorGroup("Fade Out", true, 30)]
		...
		[Condition("FadeOutOnStop", true)]
		public float FadeOutDuration = 1f;
		/// if fading out, the tween over which to fade the sound 
		[Tooltip("if fading out, the tween over which to fade the sound ")]
		public TweenType FadeOutTween = new TweenType(FeelTween.TweenCurve.EaseInOutQuartic, "FadeOutOnStop");
        
		[FeedbackInspectorGroup("Solo", true, 32)]
		...
		[FeedbackInspectorGroup("Spatial Settings", true, 33)]
		...
		[FeedbackInspectorGroup("Effects", true, 36)]
		...
		[FeedbackInspectorGroup("Time Options", true, 15)]
		...
		[FeedbackInspectorGroup("3D Sound Settings", true, 37)]
		...
		[FeedbackCondition("UseCustomRolloffCurve", true)]
		...
		[FeedbackCondition("UseSpatialBlendCurve", true)]
		...
		[FeedbackCondition("UseReverbZoneMixCurve", true)]
		...
		[FeedbackCondition("UseSpreadCurve", true)]
		public AnimationCurve SpreadCurve;
        
		[FeedbackInspectorGroup("Debug", true, 31)]
		/// whether or not to draw sound falloff gizmos when this Feedback Player is selected
		[Tooltip("whether or not to draw sound falloff gizmos when this Feedback Player is selected")]
		public bool DrawGizmos = false;
		/// an object to use as the center of the gizmos. If left empty, this Feedback Player's position will be used.
		[Tooltip("an object to use as the center of the gizmos. If left empty, this Feedback Player's position will be used.")]
		[FeedbackCondition("DrawGizmos", true)]
		...
		[FeedbackCondition("DrawGizmos", true)]
		public Color MinDistanceColor = FeelColors.CadetBlue;
		...
		[FeedbackCondition("DrawGizmos", true)]
		public Color MaxDistanceColor = FeelColors.Orangered;
		/// a test button used to play the sound in inspector
		public FeedbackButton TestPlayButton;
		/// a test button used to stop the sound in inspector
		public FeedbackButton TestStopButton;
		/// a test button used to stop the sound in inspector
		public FeedbackButton ResetSequentialIndexButton;
        
		protected AudioClip _randomClip;
		protected AudioSource _editorAudioSource;
		protected SoundManagerPlayOptions _options;
		...
		protected FeelShufflebag<int> _randomUniqueShuffleBag;
		...
		protected override void CustomInitialization(FeedbackPlayer owner)
		{
			...
			if (RandomUnique)
			{
				_randomUniqueShuffleBag = new FeelShufflebag<int>(RandomSfx.Length);
		...
		public override void InitializeCustomAttributes()
		{
			...
			TestPlayButton = new FeedbackButton("Debug Play Sound", TestPlaySound);
			TestStopButton = new FeedbackButton("Debug Stop Sound", TestStopSound);
			ResetSequentialIndexButton = new FeedbackButton("Reset Sequential Index", ResetSequentialIndex);
		}
		...
		protected virtual IEnumerator FadeOutCo()
		{
			...
			FeelSoundManager.Instance.FadeSound(_playedAudioSource, FadeOutDuration, _playedAudioSource.volume, 0f, FadeOutTween);
			yield return FeelCoroutine.WaitFor(FadeOutDuration);
			StopSound();
		}
		...
		protected virtual void StopSound()
		{
			if (StopSoundOnFeedbackStop && (_playedAudioSource != null))
			{
				...
				{
					FeelSoundManager.Instance.FreeSound(_playedAudioSource);	
				}
		...
		protected virtual void PlaySound(AudioClip sfx, Vector3 position, float intensity, AudioResource audioResource = null)
		{
			...
			{
				if ((FeelSoundManager.Instance.FindByClip(sfx) != null) && (FeelSoundManager.Instance.FindByClip(sfx).isPlaying))
				{
					return;
				}
			}
			...
			{
				if (FeelSoundManager.Instance.CurrentlyPlayingCount(sfx) >= MaximumConcurrentInstances)
				{
					return;
				}
			}
			...
			_playedAudioSource = SoundManagerSoundPlayEvent.Trigger(sfx, _options);
		...
		/// <summary>
		/// Automatically tries to add a FeelSoundManager to the scene if none are present
		/// </summary>
		public override void AutomaticShakerSetup()
		{
			FeelSoundManager soundManager = (FeelSoundManager)Object.FindAnyObjectByType(typeof(FeelSoundManager));
			if (soundManager == null)
			{
				GameObject soundManagerGo = new GameObject("FeelSoundManager");
				soundManagerGo.AddComponent<FeelSoundManager>();
				FeelDebug.DebugLogInfo( "Added a FeelSoundManager to the scene. You're all set.");
			}
		}
		...
		public virtual async void TestPlaySound()
		{
			...
			{
				temporaryAudioHost.AddComponent<ForceDestroyInPlayMode>();
			}
