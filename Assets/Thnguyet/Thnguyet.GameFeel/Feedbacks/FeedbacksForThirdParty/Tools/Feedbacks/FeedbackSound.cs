...
namespace Thnguyet.GameFeel.Feedbacks
{
	...
	[FeedbackHelp("WARNING: this is a very simple feedback, that will let you play a sound. Nothing wrong with it being simple of course, but if you want more features, you'll want to look at the FeelSoundManager Sound feedback.\n\nThis feedback lets you play the specified AudioClip, either via event (you'll need something in your scene to catch a SfxEvent, for example a FeelSoundManager), or cached (AudioSource gets created on init, and is then ready to be played), or on demand (instantiated on Play). For all these methods you can define a random volume between min/max boundaries (just set the same value in both fields if you don't want randomness), random pitch, and an optional AudioMixerGroup.")]
	public class FeedbackSound : FeedbackFeedback
	{
		...
		/// Event : sends a SfxEvent, you'll need a class to catch this event and play the sound
		...
		[FeedbackInspectorGroup("Play Method", true, 27)]
		...
		[FeedbackEnumCondition("PlayMethod", (int)PlayMethods.Pool)]
		public int PoolSize = 10;
		/// in event mode, whether to use legacy events (SfxEvent) or the current events (SoundManagerSoundPlayEvent)
		[Tooltip("in event mode, whether to use legacy events (SfxEvent) or the current events (SoundManagerSoundPlayEvent)")]
		[FeedbackEnumCondition("PlayMethod", (int)PlayMethods.Event)]
		...
		[FeedbackInspectorGroup("Sound Properties", true, 28)]
		...
		[FeedbackInspectorGroup("Spatial Settings", true, 33, false, true)]
		...
		[FeedbackInspectorGroup("3D Sound Settings", true, 37, false, true)]
		...
		protected virtual void PlaySound(AudioClip sfx, Vector3 position, float intensity)
		{
			...
			switch (PlayMethod)
			{
				case PlayMethods.Event:
					...
					{
						SfxEvent.Trigger(sfx, SfxAudioMixerGroup, volume, pitch, Priority);
					}
					else
					{
						SoundManagerPlayOptions options = new SoundManagerPlayOptions();
						options = SoundManagerPlayOptions.Default;
						...
						options.MmSoundManagerTrack = FeelSoundManager.SoundManagerTracks.Sfx;
						options.Loop = false;
						_audioSource = SoundManagerSoundPlayEvent.Trigger(sfx, options);	
					}
					break;
		...
		public override void AutomaticShakerSetup()
		{
			...
			FeelSoundManager soundManager = (FeelSoundManager)UnityEngine.Object.FindAnyObjectByType(typeof(FeelSoundManager));
