...
namespace Thnguyet.GameFeel.Feedbacks
{
	...
	public class FeedbackTrailRenderer : FeedbackFeedback
	{
		...
		[FeedbackInspectorGroup("Trail Renderer", true, 24, true)]
		...
		[FeedbackEnumCondition("Mode", (int)Modes.OverTime)]
		public TweenType Transition = new TweenType(new AnimationCurve(new Keyframe(0, 0), new Keyframe(0.3f, 1f), new Keyframe(1, 0)));

		[FeedbackInspectorGroup("Width", true, 25)]
		...
		[FeedbackInspectorGroup("Color", true, 28)]
		...
		[FeedbackInspectorGroup("Trail Renderer Time", true, 28)]
		...
		protected virtual void SetTrailRendererValues(float time, float intensityMultiplier)
		{
			if (ModifyColor)
			{
				...
				{
					TargetTrailRenderer.colorGradient = FeelColors.LerpGradients(_initialColor, NewColor, time);	
				}
				else
				{
					TargetTrailRenderer.colorGradient = FeelColors.LerpGradients(NewColor, _firstColor, time);
				}
			}
			...
			if (ModifyWidth)
			{
				if (NormalPlayDirection)
				{
					TargetTrailRenderer.widthCurve = AnimationCurves.LerpAnimationCurves(_initialWidth, NewWidth, time);	
				}
				else
				{
					TargetTrailRenderer.widthCurve = AnimationCurves.LerpAnimationCurves(NewWidth, _firstWidth, time);
				}
			}
			...
			if (ModifyTime)
			{
				if (NormalPlayDirection)
				{
					TargetTrailRenderer.time = FeelMaths.Lerp(_initialTime, NewTime, time, FeedbackDeltaTime);	
				}
				else
				{
					TargetTrailRenderer.time = FeelMaths.Lerp(NewTime, _firstTime, time, FeedbackDeltaTime);
				}
