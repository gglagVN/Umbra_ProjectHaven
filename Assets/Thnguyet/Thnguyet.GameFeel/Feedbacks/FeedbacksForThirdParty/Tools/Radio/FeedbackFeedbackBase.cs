...
{
	public class FeedbackFeedbackBaseTarget
	{
		/// the receiver to write the level to
		public PropertyReceiver Target;
		/// the curve to tween the intensity on
		public TweenType LevelCurve = new TweenType(new AnimationCurve(new Keyframe(0, 0), new Keyframe(0.3f, 1f), new Keyframe(1, 0)));
	...
	public abstract class FeedbackFeedbackBase : FeedbackFeedback
	{
		...
		[FeedbackInspectorGroup("Mode", true, 64)]
		...
		[FeedbackEnumCondition("Mode", (int)Modes.OverTime, (int)Modes.ToDestination)]
		...
		protected List<FeedbackFeedbackBaseTarget> _targets;
		...
		{
			_targets = new List<FeedbackFeedbackBaseTarget>();
			FillTargets();
			InitializeTargets();
		}
		...
		protected virtual void InitializeTargets()
		{
			...
			foreach(FeedbackFeedbackBaseTarget target in _targets)
			{
		...
		protected virtual void SetValues(float time, float feedbacksIntensity, Vector3 position, Modes mode = Modes.OverTime)
		{
			...
			foreach (FeedbackFeedbackBaseTarget target in _targets)
			{
				float intensity = FeelTween.Tween(time, 0f, 1f, target.RemapLevelZero, target.RemapLevelOne, target.LevelCurve);
				
				if (mode == Modes.ToDestination)
				{
					intensity = FeelTween.Tween(time, 0f, 1f, target.InitialLevel, target.ToDestinationLevel, target.LevelCurve);
				}
