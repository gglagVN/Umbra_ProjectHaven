...
namespace Thnguyet.GameFeel.ThirdParty
{
	...
	/// </summary>
	[AddComponentMenu("")]
	public class FeedbackUIToolkitFloatBase : FeedbackUIToolkit
	{
		...
		public TweenType Curve = new TweenType(FeelTween.TweenCurve.EaseInCubic, "", "Mode", (int)Modes.Interpolate, (int)Modes.ToDestination);
		...
		protected virtual void ApplyTime(float time)
		{
			...
			if (Mode == Modes.Interpolate)
			{
				...
				newValue = FeelTween.Tween(time, 0f, 1f, startValue, endValue, Curve);    
			}
			else if (Mode == Modes.ToDestination)
			{
				newValue = FeelTween.Tween(time, 0f, 1f, _initialValue, DestinationValue, Curve);
			}
