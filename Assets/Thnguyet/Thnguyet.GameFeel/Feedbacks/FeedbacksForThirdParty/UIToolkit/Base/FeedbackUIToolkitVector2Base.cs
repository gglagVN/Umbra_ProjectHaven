...
namespace Thnguyet.GameFeel.ThirdParty
{
	...
	/// </summary>
	[AddComponentMenu("")]
	public class FeedbackUIToolkitVector2Base : FeedbackUIToolkit
	{
		...
		[FeedbackInspectorGroup("Value", true, 16)]
		...
		[FeedbackEnumCondition("Mode", (int)Modes.Interpolate, (int)Modes.Instant)]
		...
		[FeedbackEnumCondition("Mode", (int)Modes.Interpolate, (int)Modes.ToDestination)]
		...
		public TweenType CurveX = new TweenType(FeelTween.TweenCurve.EaseInCubic, "", "Mode", (int)Modes.Interpolate, (int)Modes.ToDestination);
		/// the value to which the curve's 0 should be remapped
		[Tooltip("the value to which the curve's 0 should be remapped")]
		[FeedbackEnumCondition("Mode", (int)Modes.Interpolate)]
		...
		/// the curve to use when interpolating towards the destination value
		[Tooltip("the curve to use when interpolating towards the destination value")]
		public TweenType CurveY = new TweenType(FeelTween.TweenCurve.EaseInCubic, "", "Mode", (int)Modes.Interpolate, (int)Modes.ToDestination);
		/// the value to which the curve's 0 should be remapped
		[Tooltip("the value to which the curve's 0 should be remapped")]
		[FeedbackEnumCondition("Mode", (int)Modes.Interpolate)]
		...
		protected virtual void ApplyTime(float time)
		{
			...
			if (Mode == Modes.Interpolate)
			{
				if (AnimateX)
				{
					...
					_newValue.x = FeelTween.Tween(time, 0f, 1f, startValueX, endValueX, CurveX);	
				}
				...
				if (AnimateY)
				{
					...
					_newValue.y = FeelTween.Tween(time, 0f, 1f, startValueY, endValueY, CurveY);	
				}
			}
			else if (Mode == Modes.ToDestination)
			{
				if (AnimateX)
				{
					_newValue.x = FeelTween.Tween(time, 0f, 1f, _initialValue.x, DestinationValueX, CurveX);	
				}
				if (AnimateY)
				{
					_newValue.y = FeelTween.Tween(time, 0f, 1f, _initialValue.y, DestinationValueY, CurveY);	
				}
			}
