...
{
	/// <summary>
	/// Add this to a light 2D to have it receive LightShakeEvents from feedbacks or to shake it locally
	/// </summary>
	#if GAMEFEEL_URP
	[AddComponentMenu("Thnguyet/GameFeel/Feedbacks/Shakers/Lights/Light 2DShaker URP")]
	[RequireComponent(typeof(Light2D))]
	#endif
	public class Light2DShaker_URP : FeelShaker
	{
		#if GAMEFEEL_URP
		[InspectorGroup("Light", true, 37)]
		...
		[InspectorGroup("Intensity", true, 40)]
		...
		[InspectorGroup("Shadow Strength", true, 38)]
		/// the range to apply to the light over time
		[Tooltip("the range to apply to the light over time")]
		...
		{
			base.StartListening();
			Light2DShakeEvent.Register(OnMMLight2DShakeEvent);
		}
		...
		{
			base.StopListening();
			Light2DShakeEvent.Unregister(OnMMLight2DShakeEvent);
		}
	...
	public struct Light2DShakeEvent
	{
