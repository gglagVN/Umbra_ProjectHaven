...
namespace Thnguyet.GameFeel
{
	...
	/// </summary>
	[AddComponentMenu("Thnguyet/GameFeel/Tools/Camera/Orbital Camera")]
	public class OrbitalCamera : MonoBehaviour
	{
		...
		public virtual void Initialization()
		{
			...
			if (Target == null)
			{
				Debug.LogError(this.gameObject.name + " : the OrbitalCamera doesn't have a target.");
				return;
			}
		...
		protected virtual void Rotation()
		{
			...
			if (Mode == Modes.Touch && (Input.touchCount > 0))
			{
				if ((Input.touches[0].phase == TouchPhase.Moved) && (Input.touchCount == 1))
				{
					...
					_angleY = FeelMaths.ClampAngle(_angleY, MinVerticalAngleLimit, MaxVerticalAngleLimit);
