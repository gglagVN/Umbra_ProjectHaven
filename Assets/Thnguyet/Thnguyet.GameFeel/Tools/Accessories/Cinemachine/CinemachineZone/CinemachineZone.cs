...
namespace Thnguyet.GameFeel
{
	...
	[AddComponentMenu("")]
	[ExecuteAlways]
	public abstract class CinemachineZone : MonoBehaviour
	{
		...
		[InspectorButton("ManualSetupConfiner")]
		...
		{
			foreach (var zone in FindObjectsByType<CinemachineZone>(FindObjectsSortMode.None))
			{
				zone.Awake();
			}
		}
		...
		protected virtual void Initialization()
		{
			...
			if (VirtualCamera == null)
			{
				Debug.LogWarning("[CinemachineZone2D] " + this.name + " : no virtual camera is attached to this zone. Set one in its inspector.");
			}
			#endif
		...
		protected virtual IEnumerator EnableCamera(bool state, int frames)
		{
			...
			if (frames > 0)
			{
				yield return FeelCoroutine.WaitForFrames(frames);    
			}
		...
		{
			GizmosColor = FeelColors.RandomColor();
			GizmosColor.a = 0.2f;
		}
	}    
}
