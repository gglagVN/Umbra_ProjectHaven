...
namespace Thnguyet.GameFeel
{
	...
	/// </summary>
	[Serializable]
	[AddComponentMenu("Thnguyet/GameFeel/Tools/Cinemachine/Gyro Cam")]
	public class GyroCam
	{
		#if GAMEFEEL_CINEMACHINE
	...
	/// </summary>
	public class GyroParallax : FeelGyroscope
	{
		[Header("Cameras")]
		/// the list of cameras to move as the gyro moves
		public List<GyroCam> Cams;
		...
		protected virtual void MoveCameras()
		{
			foreach (GyroCam cam in Cams)
			{
				...
				if (gyroGravity.x > 0)
				{
					newX = FeelMaths.Remap(LerpedCalibratedGyroscopeGravity.x, 0.5f, 0, cam.MinRotation.x, 0);
				}
				if (gyroGravity.x < 0)
				{
					newX = FeelMaths.Remap(LerpedCalibratedGyroscopeGravity.x, 0, -.5f, 0, cam.MaxRotation.x);
				}
				if (gyroGravity.y > 0)
				{
					newY = FeelMaths.Remap(LerpedCalibratedGyroscopeGravity.y, 0.5f, 0, cam.MinRotation.y, 0f);
				}
				if (gyroGravity.y < 0)
				{
					newY = FeelMaths.Remap(LerpedCalibratedGyroscopeGravity.y, 0f, -0.5f, 0f, cam.MaxRotation.y);
				}
