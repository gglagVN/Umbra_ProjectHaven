...
{	
	...
	public class DebugMenuRadioButton : DebugMenuSpriteReplace 
	{
		...
		{
			_group = new List<DebugMenuRadioButton> ();

			DebugMenuRadioButton[] radioButtons = FindObjectsByType(typeof(DebugMenuRadioButton), FindObjectsSortMode.None) as DebugMenuRadioButton[];
			foreach (DebugMenuRadioButton radioButton in radioButtons) 
			{
		...
		protected override void SpriteOn()
		{
			...
			if (_group.Count >= 1)
			{
				foreach (DebugMenuRadioButton radioButton in _group) 
				{
