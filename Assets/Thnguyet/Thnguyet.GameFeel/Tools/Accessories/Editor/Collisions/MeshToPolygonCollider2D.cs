...
namespace Thnguyet.GameFeel
{
	...
	/// </summary>
	public class MeshToPolygonCollider2D : MonoBehaviour
	{
		/// <summary>
		...
		private static Vector3[] MeshFilterToVectors(MeshFilter meshFilter)
		{
			...
			List<FeelGeometry.Edge> boundaryPath = FeelGeometry.GetEdges(meshFilter.sharedMesh.triangles).FindBoundary().SortEdges();
		...
		private static bool ValidateMesh(MeshFilter meshFilter)
		{
			...
			{
				Debug.LogWarning("[MeshToPolygonCollider2D] " 
		...
		/// </summary>
		[MenuItem("Tools/Thnguyet/GameFeel/Collisions/Generate PolygonCollider2D", false, 601)]
		public static void GeneratePolygonCollider2DMenu()
		{
			Transform activeTransform = Selection.activeTransform;
			if (activeTransform == null)
			{
				Debug.LogWarning("[MeshToPolygonCollider2D] You need to select a gameobject first.");
				return;
			}
