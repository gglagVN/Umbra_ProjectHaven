...
namespace Thnguyet.GameFeel
{
	...
	/// </summary>
	[AddComponentMenu("Thnguyet/GameFeel/Tools/GUI/Health Bar")]
	public class HealthBar : MonoBehaviour 
	{
		...
		[Information("Add this component to an object and it'll add a healthbar next to it to reflect its health level in real time. You can decide here whether the health bar should be drawn automatically or use a prefab.",Thnguyet.GameFeel.InformationAttribute.InformationType.Info,false)]
		...
		[Information("Select a prefab with a progress bar script on it. There is one example of such a prefab in Common/Prefabs/GUI.",Thnguyet.GameFeel.InformationAttribute.InformationType.Info,false)]
		/// the prefab to use as the health bar
		[Tooltip("the prefab to use as the health bar")]
		public FeelProgressBar HealthBarPrefab;

		[Header("Existing FeelProgressBar")]
		/// the FeelProgressBar this health bar should update 
		[Tooltip("the FeelProgressBar this health bar should update")]
		public FeelProgressBar TargetProgressBar;

		[Header("Drawn Healthbar Settings ")]
		[Information("Set the size (in world units), padding, back and front colors of the healthbar.",Thnguyet.GameFeel.InformationAttribute.InformationType.Info,false)]
		...
		/// the rotation to apply to the HealthBarContainer when drawing it
		[Tooltip("the rotation to apply to the HealthBarContainer when drawing it")]
		...
		public Gradient ForegroundColor = new Gradient()
		{
			colorKeys = new GradientColorKey[2] {
				new GradientColorKey(FeelColors.BestRed, 0),
				new GradientColorKey(FeelColors.BestRed, 1f)
			},
		...
		{
			colorKeys = new GradientColorKey[2] {
				new GradientColorKey(FeelColors.Orange, 0),
				new GradientColorKey(FeelColors.Orange, 1f)
			},
			alphaKeys = new GradientAlphaKey[2] { new GradientAlphaKey(1, 0), new GradientAlphaKey(1, 1) }
		};
		...
		{
			colorKeys = new GradientColorKey[2] {
				new GradientColorKey(FeelColors.AntiqueWhite, 0),
				new GradientColorKey(FeelColors.AntiqueWhite, 1f)
			},
			alphaKeys = new GradientAlphaKey[2] { new GradientAlphaKey(1, 0), new GradientAlphaKey(1, 1) }
		};
		...
		{
			colorKeys = new GradientColorKey[2] {
				new GradientColorKey(FeelColors.Black, 0),
				new GradientColorKey(FeelColors.Black, 1f)
			},
			alphaKeys = new GradientAlphaKey[2] { new GradientAlphaKey(1, 0), new GradientAlphaKey(1, 1) }
		};
		...
		/// if this is true, the drawn health bar will be nested below the HealthBar
		[Tooltip("if this is true, the drawn health bar will be nested below the HealthBar")]
		public bool NestDrawnHealthBar = false;
		/// if this is true, a FeelBillboard component will be added to the progress bar to make sure it always looks towards the camera
		[Tooltip("if this is true, a FeelBillboard component will be added to the progress bar to make sure it always looks towards the camera")]
		...
		[Header("Offset")]
		[Information("Set the offset (in world units), relative to the object's center, to which the health bar will be displayed.",Thnguyet.GameFeel.InformationAttribute.InformationType.Info,false)]
		...
		[Header("Display")]
		[Information("Here you can define whether or not the healthbar should always be visible. If not, you can set here how long after a hit it'll remain visible.",Thnguyet.GameFeel.InformationAttribute.InformationType.Info,false)]
		...
		[InspectorButton("TestUpdateHealth")]
		...
		public virtual void Initialization()
		{
			...
			switch (HealthBarType)
			{
				case HealthBarTypes.Prefab:
					...
					_progressBar = Instantiate(HealthBarPrefab, transform.position + HealthBarOffset, transform.rotation) as FeelProgressBar;
		...
		protected virtual void DrawHealthBar()
		{
			...
			_progressBar = newGameObject.AddComponent<FeelProgressBar>();
			...
			if (Billboard)
			{
				FeelBillboard billboard = _progressBar.gameObject.AddComponent<FeelBillboard>();
			...
			_progressBar.TimeScale = (TimeScale == TimeScales.Time) ? FeelProgressBar.TimeScales.Time : FeelProgressBar.TimeScales.UnscaledTime;
