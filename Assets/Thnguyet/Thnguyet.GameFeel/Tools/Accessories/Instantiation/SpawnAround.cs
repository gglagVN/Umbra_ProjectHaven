...
namespace Thnguyet.GameFeel
{
	...
	public class SpawnAroundProperties
	{
		...
		[EnumCondition("Shape", (int)SpawnAroundShapes.Sphere)]
		...
		[EnumCondition("Shape", (int)SpawnAroundShapes.Cube)]
		...
		[Condition("UseNormalAxisOffsetCurve",true)]
	...
	/// instantiate objects  
	/// </summary>
	public static class SpawnAround 
	{
		public static void ApplySpawnAroundProperties(GameObject instantiatedObj, SpawnAroundProperties props, Vector3 origin)
		{            
		...
		/// <returns></returns>
		public static Vector3 SpawnAroundPosition(SpawnAroundProperties props, Vector3 origin)
		{
			// we get the position of the object based on the defined plane and distance
			Vector3 newPosition;
			if (props.Shape == SpawnAroundProperties.SpawnAroundShapes.Sphere)
			{
			...
			if (props.UseNormalAxisOffsetCurve)
			{
				...
				if (randomOffset != 0)
				{
					...
					{
						normalizedOffset = FeelMaths.Remap(randomOffset, props.MinimumNormalAxisOffset, props.MaximumNormalAxisOffset, 1f, 0f);
					}
					else
					{
						normalizedOffset = FeelMaths.Remap(randomOffset, props.MinimumNormalAxisOffset, props.MaximumNormalAxisOffset, 0f, 1f);
					}
				}

				float offset = props.NormalOffsetCurve.Evaluate(normalizedOffset);
				offset = FeelMaths.Remap(offset, 0f, 1f, props.NormalOffsetCurveRemapZero, props.NormalOffsetCurveRemapOne);
		...
		public static Vector3 PickPositionInsideCube(SpawnAroundProperties props)
		{
			...
			while (iterationsCount < maxIterationsCount)
			{
				...
				{
					randomX = FeelMaths.RollADice(2) > 1 ? -randomX : randomX;
					randomY = FeelMaths.RollADice(2) > 1 ? -randomY : randomY;
					randomZ = FeelMaths.RollADice(2) > 1 ? -randomZ : randomZ;
		...
		/// <param name="props"></param>
		/// <returns></returns>
		public static Vector3 SpawnAroundScale(SpawnAroundProperties props)
		{
			return FeelMaths.RandomVector3(props.MinimumScale, props.MaximumScale);
		}
		...
		/// <param name="props"></param>
		/// <returns></returns>
		public static Quaternion SpawnAroundRotation(SpawnAroundProperties props)
		{
			return Quaternion.Euler(FeelMaths.RandomVector3(props.MinimumRotation, props.MaximumRotation));
		}
		...
		public static void DrawGizmos(SpawnAroundProperties props, Vector3 origin, int quantity, float size, Color gizmosColor)
		{
