...
namespace Thnguyet.GameFeel
{
	...
	public class FeelAim
	{
		...
		[Information("Pick a control mode : mouse (aims towards the pointer), primary movement (you'll aim towards the current input direction), or secondary movement (aims " +
		               "towards a second input axis, think twin stick shooters), and set minimum and maximum angles.", Thnguyet.GameFeel.InformationAttribute.InformationType.Info, false)]
		...
		public virtual Vector2 GetCurrentAim()
		{
			...
			{
				CurrentAngle = FeelMaths.RoundToClosest(CurrentAngle, _possibleAngleValues);
			}
			...
			_currentAim = (_currentAim.magnitude == 0f) ? Vector2.zero : FeelMaths.RotateVector2(Vector2.right, CurrentAngle);
			return _currentAim;
		}
