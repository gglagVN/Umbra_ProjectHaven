...
namespace Thnguyet.GameFeel
{
	...
	/// </summary>
	[AddComponentMenu("Thnguyet/GameFeel/Tools/Movement/Squash And Stretch")]
	public class SquashAndStretch : MonoBehaviour
	{
		...
		protected virtual void ComputeVelocityAndDirection()
		{
			...
			RemappedVelocity = FeelMaths.Remap(VelocityMagnitude, 0f, MaximumVelocity, 0f, 1f);
			...
			if (AutoSquashOnStop)
			{
				...
				else if (_movementStarted)
				{
					...
					_squashing = true;
					float duration = FeelMaths.Remap(_lastVelocity, 0f, MaximumVelocity, SquashDuration.x, SquashDuration.y);
					float intensity = FeelMaths.Remap(_lastVelocity, 0f, MaximumVelocity, SquashIntensity.x, SquashIntensity.y);
		...
		protected virtual void ComputeNewLocalScale()
		{
			if (_squashing)
			{
				float elapsed = FeelMaths.Remap(TimescaleTime - _squashStartedAt, 0f, _squashDuration, 0f, 1f);
			...
			if (Spring)
			{
				FeelMaths.Spring(ref _springScale, _newLocalScale, ref _springVelocity, SpringDamping, SpringFrequency, Time.deltaTime);
