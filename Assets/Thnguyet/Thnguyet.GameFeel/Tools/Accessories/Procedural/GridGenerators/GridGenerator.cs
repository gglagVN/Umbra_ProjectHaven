...
namespace Thnguyet.GameFeel
{
	public class GridGenerator
	{
		...
		public static int[,] TilemapToGrid(Tilemap tilemap, int width, int height)
		{
			if(tilemap == null)
			{
				Debug.LogError("[GridGenerator] You're trying to convert a tilemap into a grid but didn't specify what tilemap to convert.");
				return null;
			}
			...
			for (int i = 0; i < width; i++)
			{
				for (int j = 0; j < height; j++)
				{
					currentPosition.x = i;
					currentPosition.y = j;
					currentPosition += TilemapGridRenderer.ComputeOffset(width-1, height-1);
		...
		/// <returns></returns>
		public static int[,] ApplySafeSpots(int[,] grid, List<TilemapGeneratorLayer.TilemapGeneratorLayerSafeSpot> safeSpots)
		{
			foreach (TilemapGeneratorLayer.TilemapGeneratorLayerSafeSpot safeSpot in safeSpots)
			{
