...
namespace Thnguyet.GameFeel 
{
	...
	public class TilemapGenerator : MonoBehaviour
	{
		...
		public TilemapGeneratorLayerList Layers;
		...
		public TweenType SlowRenderTweenType = new TweenType(FeelTween.TweenCurve.EaseInOutCubic);
		...
		public virtual void Generate()
		{
			...
			foreach (TilemapGeneratorLayer layer in Layers)
			{
				GenerateLayer(layer);   
			}
		}
		...
		void Reset()
		{
			Layers = new TilemapGeneratorLayerList(){
				new TilemapGeneratorLayer()
			};
		}
		...
		protected virtual void GenerateLayer(TilemapGeneratorLayer layer)
		{
			if (!layer.Active)
			{
				return;
			...
			switch (layer.GenerateMethod)
			{
				case GenerateMethods.Full:
					_grid = GridGeneratorFull.Generate(width, height, layer.FullGenerationFilled);
					layer.Grid = _grid;
					break;
				case GenerateMethods.Perlin:
					_grid = GridGeneratorPerlinNoise.Generate(width, height, seedFloat);
					layer.Grid = _grid;
					break;
				case GenerateMethods.PerlinGround:
					_grid = GridGeneratorPerlinNoiseGround.Generate(width, height, seedFloat);
					layer.Grid = _grid;
					break;
				case GenerateMethods.Random:
					_grid = GridGeneratorRandom.Generate(width, height, seed, layer.RandomFillPercentage);
					layer.Grid = _grid;
					break;
				case GenerateMethods.RandomWalk:
					_grid = GridGeneratorRandomWalk.Generate(width, height, seed, layer.RandomWalkPercent, layer.RandomWalkStartingPoint, layer.RandomWalkMaxIterations, layer.BoundsTop, layer.BoundsBottom, layer.BoundsLeft, layer.BoundsRight);
					layer.Grid = _grid;
					break;
				case GenerateMethods.RandomWalkAvoider:

					int[,] obstacleGrid = GridGenerator.TilemapToGrid(layer.RandomWalkAvoiderObstaclesTilemap, width, height);
					_grid = GridGeneratorRandomWalkAvoider.Generate(width, height, seed, layer.RandomWalkAvoiderPercent, layer.RandomWalkAvoiderStartingPoint, obstacleGrid, layer.RandomWalkAvoiderObstaclesDistance, layer.RandomWalkAvoiderMaxIterations);
					layer.Grid = _grid;
					break;
				case GenerateMethods.RandomWalkGround:
					_grid = GridGeneratorRandomWalkGround.Generate(width, height, seed, 
				...
				case GenerateMethods.Path:
					_grid = GridGeneratorPath.Generate(width, height, seed, layer.PathDirection, layer.PathStartPosition, layer.PathMinWidth,
			...
			if (layer.Smooth) { _grid = GridGenerator.SmoothenGrid(_grid); }
			if (layer.InvertGrid) { _grid = GridGenerator.InvertGrid(_grid); }

			_grid = GridGenerator.BindGrid(_grid, layer.BoundsTop, layer.BoundsBottom, layer.BoundsLeft, layer.BoundsRight);
			_grid = GridGenerator.ApplySafeSpots(_grid, layer.SafeSpots);
		...
		/// </summary>
		/// <param name="layer"></param>
		async static void DelayedCopy(TilemapGeneratorLayer layer)
		{
			await Task.Delay(500);
			TilemapShadow.Copy(layer.CopyTilemap, layer.TargetTilemap);
		}
		...
		/// </summary>
		/// <param name="layer"></param>
		protected virtual void RenderGrid(TilemapGeneratorLayer layer)
		{
			TilemapGridRenderer.RenderGrid(_grid, layer, SlowRender, SlowRenderDuration, SlowRenderTweenType,this);
		}
