...
namespace Thnguyet.GameFeel
{
	public class TilemapGridRenderer 
	{
		...
		public static void RenderGrid(int[,] grid, TilemapGeneratorLayer layer, bool slowRender = false, float slowRenderDuration = 1f, 
			TweenType slowRenderTweenType = null, MonoBehaviour slowRenderSupport = null)
		{
			if (layer.FusionMode == TilemapGeneratorLayer.FusionModes.Normal)
			{
				ClearTilemap(layer.TargetTilemap);    
			}
			TileBase tile = layer.Tile;
			if (layer.FusionMode == TilemapGeneratorLayer.FusionModes.Combine)
			{
				grid = GridGenerator.InvertGrid(grid);
				tile = null;
			}
			if (layer.FusionMode == TilemapGeneratorLayer.FusionModes.Subtract)
			{
				grid = GridGenerator.InvertGrid(grid);
			}
		...
		/// <returns></returns>
		public static IEnumerator SlowRenderGrid(int[,] grid, Tilemap tilemap, TileBase tile, float slowRenderDuration, TweenType slowRenderTweenType, int frameRate)
		{
			...
			while (Time.unscaledTime - startedAt < slowRenderDuration)
			{
				...
				float normalizedProgress = FeelMaths.Remap(elapsedTime, 0f, slowRenderDuration, 0f, 1f);
				float curveProgress =  FeelTween.Tween(normalizedProgress, 0f, 1f, 0f, 1f, slowRenderTweenType);
