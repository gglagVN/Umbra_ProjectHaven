...
namespace Thnguyet.GameFeel
{
	...
	public class ExecutionOrderAttribute : Attribute
	{
		...
		protected static Dictionary<MonoScript, ExecutionOrderAttribute> _monoScripts;
		...
		public ExecutionOrderAttribute(int newExecutionOrder)
		{
		...
		{
			_monoScripts = new Dictionary<MonoScript, ExecutionOrderAttribute>();
			_executionOrderAttributeType = typeof(ExecutionOrderAttribute);
		...
		protected static void FindExecutionOrderAttributes()
		{
			foreach (Type assemblyType in _assemblyTypes)
			{
				...
				ExecutionOrderAttribute attribute = attributes[0] as ExecutionOrderAttribute;
				...
				{
					Debug.LogError("Tools' ExecutionOrderAttribute : Can't change "+ assemblyType.Name + "'s execution order");
					return;
				}
		...
		protected static bool ExecutionOrderHasChanged()
		{
			...
			foreach (KeyValuePair<MonoScript, ExecutionOrderAttribute> monoScript in _monoScripts)
			{
