...
namespace Thnguyet.GameFeel
{
	...
	public class Beat
	{
		...
		public void InitializeIfNeeded(int id, int bandID)
		{
			if (!_initialized)
			{
				...
				Name = "Beat " + id;
				BeatColor = FeelColors.RandomColor();
				BandID = bandID;
	...
	/// </summary>
	[AddComponentMenu("Thnguyet/GameFeel/Tools/Audio/Audio Analyzer")]
	public class AudioAnalyzer : MonoBehaviour
	{
		...
		[Information("This component lets you pick an audio source (either global : the whole scene's audio, a unique source, or the " +
		               ...
		               "will determine in how many bands you want to cut the sound. The more bands, the more levers you'll have to play with afterwards." +
		               "In general, for all of these settings, higher values mean better quality and lower performance. The buffer speed determines how " +
		               "fast buffered band levels readjust.", Thnguyet.GameFeel.InformationAttribute.InformationType.Info, false)]
		[ReadOnlyWhenPlaying]
		...
		public virtual void Initialization()
		{
			...
			{
				Debug.LogError(this.name + " : this AudioAnalyzer needs a target audio source to operate.");
				return;
			}
		...
		protected virtual void OnBeat(Beat beat, float rawValue)
		{
			...
			BeatEvent.Trigger(beat.Name, beat.CurrentValue);
