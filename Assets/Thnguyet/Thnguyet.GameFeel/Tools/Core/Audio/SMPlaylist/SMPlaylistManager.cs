...
{
	public class SMPlaylistManager : FeelMonoBehaviour
	{
		...
		[InspectorGroup("Settings", true, 18)]
		...
		public TweenType FadeTween = new TweenType(FeelTween.TweenCurve.EaseInCubic);

		[InspectorGroup("Time", true, 20)] 
		...
		[Condition("BindPitchToTimeScale", true)]
		...
		[FeelReadOnly]
		public FeelStateMachine<PlaylistManagerStates> PlaylistManagerState;
		...
		[InspectorGroup("Test Controls", true, 15)]
		/// a play test button
		...
		[InspectorButton("PlayPreviousSong")]
		public bool PreviousButton;
		/// a next song test button
		[InspectorButton("PlayNextSong")]
		...
		/// a next song test button
		[InspectorButton("PlayTargetSong")]
		public bool TargetSongButton;
		/// a next song test button
		[InspectorButton("QueueTargetSong")]
		public bool QueueTargetSongButton;
		/// a next song test button
		[InspectorButton("SetCurrentSongToLoop")]
		public bool SetLoopTargetSongButton;
		/// a next song test button
		[InspectorButton("StopCurrentSongFromLooping")]
		...
		public static SMPlaylistManager Current => _instance;
		protected static SMPlaylistManager _instance;
		...
		#region INITIALIZATION
			...
			public static SMPlaylistManager Instance
			{
				get
				{
					if (_instance == null)
					{
						_instance = FindAnyObjectByType<SMPlaylistManager>();
						if (_instance == null)
						{
							GameObject obj = new GameObject ();
							obj.name = typeof(FeelPlaylist).Name + "_AutoCreated";
							_instance = obj.AddComponent<SMPlaylistManager> ();
						}
					}
					return _instance;
				}
			...
			{
				if (
					((Playlist.PlayOrder == SMPlaylist.PlayOrders.Random) || (Playlist.PlayOrder == SMPlaylist.PlayOrders.RandomUnique)) 
			...
			{
				PlaylistManagerState = new FeelStateMachine<PlaylistManagerStates>(this.gameObject, true);
		...
		#region LIFECYCLE
			...
			protected virtual void Update()
			{
				...
				if (FeelSoundManager.Instance.IsPaused(Playlist.Track))
				{
					return;
				}
			...
			protected virtual void HandleTimescale()
			{
				...
				{
					float remappedTimescale = FeelMaths.Remap(Time.timeScale, TimescaleRemapFrom.x, TimescaleRemapFrom.y,
		...
		#region CONTROLS
			...
			public virtual void Play()
			{
				switch (PlaylistManagerState.CurrentState)
				{
					...
					case PlaylistManagerStates.Paused:
						FeelSoundManager.Instance.ResumeSound(_currentlyPlayingAudioSource);
			...
			public virtual void PlaySongAt(int songIndex)
			{
				...
				_currentlyPlayingAudioSource = SoundManagerSoundPlayEvent.Trigger(Playlist.Songs[songIndex].Clip, _options);
				
				OnSongStart?.Invoke();

				if (FadeIn)
				{
					FeelSoundManager.Instance.FadeSound(_currentlyPlayingAudioSource, FadeDuration,
				...
				PlaylistNewSongStartedEvent.Trigger(Channel);
			}
			...
			public virtual void Pause()
			{
				...
				FeelSoundManager.Instance.PauseSound(_currentlyPlayingAudioSource);
			...
			public virtual void Stop() 
			{
				...
				if (FadeOut)
				{
					if ( FeelSoundManager.Instance.SoundIsFadingOut(_currentlyPlayingAudioSource) )
					{
						return;
					}
				...
				{
					FeelSoundManager.Instance.FreeSound(_currentlyPlayingAudioSource);
				}
				ChangePlaylistManagerState(PlaylistManagerStates.Idle);
			...
			public virtual void ChangePlaylist(SMPlaylist newPlaylist) 
			{
			...
			/// </summary>
			/// <param name="newPlaylist"></param>
			public virtual void ChangePlaylistAndPlay(SMPlaylist newPlaylist)
			{
			...
			public virtual void SetVolumeMultiplier(float newVolumeMultiplier)
			{
				...
				PlaylistVolumeMultiplierEvent.Trigger(Channel, newVolume, true);
			}
			...
			public virtual void SetPitchMultiplier(float newPitchMultiplier)
			{
				...
				PlaylistPitchMultiplierEvent.Trigger(Channel, newPitch, true);
			}
		...
		#region EVENTS
			...
			protected virtual void OnMMPlaylistChangeEvent(int channel, SMPlaylist newPlaylist, bool andPlay)
			{
				if (channel != Channel) { return; }
				if (andPlay)
				{
			...
			public virtual void StartListening()
			{
				...
				PlaylistPauseEvent.Register(OnPauseEvent);
				PlaylistPlayEvent.Register(OnPlayEvent);
				PlaylistPlayNextEvent.Register(OnPlayNextEvent);
				PlaylistPlayPreviousEvent.Register(OnPlayPreviousEvent);
				PlaylistStopEvent.Register(OnStopEvent);
				PlaylistPlayIndexEvent.Register(OnPlayIndexEvent);
				PlaylistVolumeMultiplierEvent.Register(OnMMPlaylistVolumeMultiplierEvent);
				PlaylistPitchMultiplierEvent.Register(OnMMPlaylistPitchMultiplierEvent);
				PlaylistChangeEvent.Register(OnMMPlaylistChangeEvent);
			}
			...
			public virtual void StopListening()
			{
				...
				PlaylistPauseEvent.Unregister(OnPauseEvent);
				PlaylistPlayEvent.Unregister(OnPlayEvent);
				PlaylistPlayNextEvent.Unregister(OnPlayNextEvent);
				PlaylistPlayPreviousEvent.Unregister(OnPlayPreviousEvent);
				PlaylistStopEvent.Unregister(OnStopEvent);
				PlaylistPlayIndexEvent.Unregister(OnPlayIndexEvent);
				PlaylistVolumeMultiplierEvent.Unregister(OnMMPlaylistVolumeMultiplierEvent);
				PlaylistPitchMultiplierEvent.Unregister(OnMMPlaylistPitchMultiplierEvent);
				PlaylistChangeEvent.Unregister(OnMMPlaylistChangeEvent);
			}
			...
			protected virtual void OnValidate()
			{
				...
				{
					PlaylistVolumeMultiplierEvent.Trigger(Channel, TestVolumeControl, true);
				}
				if (_lastTestPlaybackSpeedControl != TestPlaybackSpeedControl)
				{
					PlaylistPitchMultiplierEvent.Trigger(Channel, TestPlaybackSpeedControl, true);
				}
			}
			#endif
