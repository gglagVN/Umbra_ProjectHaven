...
namespace Thnguyet.GameFeel
{
	...
	/// </summary>
	[AddComponentMenu("Thnguyet/GameFeel/Tools/Audio/Sound Manager")]
	public class FeelSoundManager : PersistentSingleton<FeelSoundManager>, 
		...
		EventListener<SoundManagerTrackFadeEvent>
	{
		...
		/// <returns></returns>
		public virtual AudioSource PlaySound(AudioClip audioClip, SoundManagerTracks mmSoundManagerTrack, Vector3 location, 
		...
		)
		{
			...
			if (!audioSource)
			{
				...
				if (!audioSource)
				{
					Debug.LogError("There are no available audiosources, this sound won't play. You should probably consider a bigger pool size, or let your pool expand by setting PoolCanExpand to true on your Sound Manager.");
					return null;
				}
			...
			if (!audioSource)
			{
				_tempAudioSourceGameObject = new GameObject("Audio_"+audioClip.name);
			...
			{
				FollowTarget followTarget = audioSource.gameObject.GetComponentNoAlloc<FollowTarget>();
				if (followTarget == null)
				{
					followTarget = audioSource.gameObject.AddComponent<FollowTarget>();
				}
		...
		/// <summary>
		/// Returns a list of SoundManagerSounds for the specified track
		/// </summary>
		...
		public virtual void FadeTrack(SoundManagerTracks track, float duration, float initialVolume = 0f, float finalVolume = 1f, TweenType tweenType = null)
		{
		...
		/// <param name="finalVolume"></param>
		/// <param name="tweenType"></param>
		public virtual void FadeSound(AudioSource source, float duration, float initialVolume, float finalVolume, TweenType tweenType, bool freeAfterFade = false)
		{
		...
		/// <param name="tweenType"></param>
		/// <returns></returns>
		protected virtual IEnumerator FadeTrackCoroutine(SoundManagerTracks track, float duration, float initialVolume, float finalVolume, TweenType tweenType)
		{
			float startedAt = Time.unscaledTime;
			if (tweenType == null)
			{
				tweenType = new TweenType(FeelTween.TweenCurve.EaseInOutQuartic);
			}
			while (Time.unscaledTime - startedAt <= duration)
			{
				float elapsedTime = Time.unscaledTime - startedAt;
				float newVolume = FeelTween.Tween(elapsedTime, 0f, duration, initialVolume, finalVolume, tweenType);
		...
		/// <param name="tweenType"></param>
		/// <returns></returns>
		protected virtual IEnumerator FadeCoroutine(AudioSource source, float duration, float initialVolume, float finalVolume, TweenType tweenType, bool freeAfterFade = false)
		{
			float startedAt = Time.unscaledTime;
			if (tweenType == null)
			{
		...
		/// <summary>
		/// Mutes all sounds playing on the FeelSoundManager
		/// </summary>
		/// <param name="mute"></param>
		...
		protected virtual IEnumerator MuteSoundsOnTrackCoroutine(SoundManagerTracks track, bool mute, float delay)
		{
			if (delay > 0)
			{
				yield return FeelCoroutine.WaitForUnscaled(delay);    
			}
		...
		/// <summary>
		/// Pauses all sounds playing on the FeelSoundManager
		/// </summary>
		...
		/// <summary>
		/// Plays all sounds playing on the FeelSoundManager
		/// </summary>
		...
		/// <summary>
		/// Stops all sounds playing on the FeelSoundManager
		/// </summary>
		...
		public virtual void OnMMEvent(SoundManagerTrackEvent soundManagerTrackEvent)
		{
		...
		public virtual void OnMMEvent(SoundManagerEvent soundManagerEvent)
		{
		...
		public virtual void OnMMEvent(SoundManagerSoundControlEvent soundControlEvent)
		{
			...
			{
				switch (soundControlEvent.SoundManagerSoundControlEventType)
				{
		...
		public virtual void OnMMEvent(SoundManagerTrackFadeEvent trackFadeEvent)
		{
		...
		public virtual void OnMMEvent(SoundManagerSoundFadeEvent soundFadeEvent)
		{
		...
		public virtual void OnMMEvent(SoundManagerAllSoundsControlEvent allSoundsControlEvent)
		{
		...
		public virtual void OnMMSfxEvent(AudioClip clipToPlay, AudioMixerGroup audioGroup = null, float volume = 1f, float pitch = 1f, int priority = 128)
		{
			...
			options.MmSoundManagerTrack = SoundManagerTracks.Sfx;
			options.Loop = false;
            
			PlaySound(clipToPlay, options);
		}

		public virtual AudioSource OnMMSoundManagerSoundPlayEvent(AudioClip clip, SoundManagerPlayOptions options)
		{
		...
		protected virtual void OnEnable()
		{
			if (_enabled)
			{
				SfxEvent.Register(OnMMSfxEvent);
				SoundManagerSoundPlayEvent.Register(OnMMSoundManagerSoundPlayEvent);
		...
		protected virtual void OnDisable()
		{
			if (_enabled)
			{
				SfxEvent.Unregister(OnMMSfxEvent);
				SoundManagerSoundPlayEvent.Unregister(OnMMSoundManagerSoundPlayEvent);
