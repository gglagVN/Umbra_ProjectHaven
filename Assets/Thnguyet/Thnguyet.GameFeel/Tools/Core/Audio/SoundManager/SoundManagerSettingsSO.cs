...
namespace Thnguyet.GameFeel
{
	...
	/// </summary>
	[Serializable]
	[CreateAssetMenu(menuName = "Thnguyet/GameFeel/Audio/SoundManagerSettings")]
	public class SoundManagerSettingsSO : ScriptableObject
	{
		...
		/// the full settings for this FeelSoundManager
		[Tooltip("the full settings for this FeelSoundManager")]
		public SoundManagerSettings Settings;

		protected const string _saveFolderName = "FeelSoundManager/";
		...
		public virtual void LoadSoundSettings()
		{
			if (Settings.OverrideMixerSettings)
			{
				SoundManagerSettings settings =
					(SoundManagerSettings) SaveLoadManager.Load(typeof(SoundManagerSettings), _saveFileName,
						_saveFolderName);
		...
		public virtual void SetTrackVolume(FeelSoundManager.SoundManagerTracks track, float volume)
		{
			if (volume <= 0f)
			{
		...
		/// <param name="track"></param>
		/// <returns></returns>
		public virtual float GetTrackVolume(FeelSoundManager.SoundManagerTracks track)
		{
			float volume = 1f;
			switch (track)
			{
		...
		{
			Settings.MasterVolume = GetTrackVolume(FeelSoundManager.SoundManagerTracks.Master);
			Settings.MusicVolume = GetTrackVolume(FeelSoundManager.SoundManagerTracks.Music);
			Settings.SfxVolume = GetTrackVolume(FeelSoundManager.SoundManagerTracks.Sfx);
			Settings.UIVolume = GetTrackVolume(FeelSoundManager.SoundManagerTracks.UI);
		}
