...
{
	#if UNITY_EDITOR
	[CustomEditor(typeof(AudioAnalyzer), true)]
	...
	public class AudioAnalyzerEditor : Editor
	{
		...
		protected Color _normalBarColor = FeelColors.Orange;
		protected Color _normalNormalizedBarColor = FeelColors.Aqua;
		protected Color _peakColor = FeelColors.Yellow;
		protected Color _activePeakColor = Color.white;
		protected Color _amplitudeColor = FeelColors.DarkOrange;
		protected Color _normalizedAmplitudeColor = FeelColors.Aquamarine;
		protected Color _spectrumColor = FeelColors.HotPink;
		...
		{
			if ((Beats == null) || (target as AudioAnalyzer).Beats == null)
			{
				return;
			}

			float length = (target as AudioAnalyzer).Beats.Length;
			if (length <= 0)
			{
				return;
			}
			...
			for (int i = 0; i < length; i++)
			{
				...
				{
					// draw front bar 
					_beatColor = (target as AudioAnalyzer).Beats[i].BeatColor;
					_beatColor.a = (target as AudioAnalyzer).Beats[i].CurrentValue;
					_rect.x = boxX;
					_rect.y = boxY;
					_rect.width = beatsBoxSquareSize;
					_rect.height = beatsBoxSquareSize;
		...
		protected virtual void DrawBandVisualization()
		{
			...
			if ((BandPeaks != null) && (BandPeaks.arraySize == _numberOfBands))
			{
				for (int i = 0; i < _numberOfBands; i++)
				{
					...
					_axisOrigin.y = _boxBottomY - FeelMaths.Remap(peak, 0f, 1f, 0f, _maxColumnHeight);
			...
			// amplitude cursors
			_columnHeight = FeelMaths.Remap(Amplitude.floatValue, 0f, 1f, 0f, _maxColumnHeight);
			...
			_columnHeight = FeelMaths.Remap(BufferedAmplitude.floatValue, 0f, 1f, 0f, _maxColumnHeight);
			...
			for (int i = 0; i < _numberOfBands; i++)
			{
				if (Active)
				{
					...
					_columnHeight = FeelMaths.Remap(bandLevel, 0f, 1f, 0f, _maxColumnHeight);
		...
		protected virtual void DrawBandVisualizationNormalized()
		{
			...
			// amplitude cursors
			_columnHeight = FeelMaths.Remap(NormalizedAmplitude.floatValue, 0f, 1f, 0f, _maxColumnHeight);
			_positionX = _boxPosition.x - _externalMargin / 4;
			_positionY = _boxBottomY - _columnHeight;
			...
			_columnHeight = FeelMaths.Remap(NormalizedBufferedAmplitude.floatValue, 0f, 1f, 0f, _maxColumnHeight);
			_positionX = _boxPosition.x + _boxSize.x - _externalMargin / 4;
			_positionY = _boxBottomY - _columnHeight;
		...
		protected virtual void DrawRawSpectrum()
		{
			...
			if (Active)
			{
				...
				for (int i = 1; i < RawSpectrum.arraySize - 1; i++)
				{
					...
					float xPosition = _spectrumBoxPosition.x + _externalMargin + FeelMaths.Remap(i, 0, RawSpectrum.arraySize, 0f, _spectrumBoxSize.x - _externalMargin * 2);
			...
			else
			{
				...
				{
					float xPosition = _spectrumBoxPosition.x + _externalMargin + FeelMaths.Remap(i, 0, points, 0f, _spectrumBoxSize.x - _externalMargin * 2);
					float yPosition = _spectrumBoxPosition.y + _spectrumBoxSize.y / 2;
		...
		{
			if ((target as AudioAnalyzer).Mode != AudioAnalyzer.Modes.AudioSource)
			{
				return;
			}
			...
			if (!PeaksPasted.boolValue)
			{
				if ((PeaksSaver.Peaks == null) || (PeaksSaver.Peaks.Length == 0))
				{
					...
					{
						(target as AudioAnalyzer).FindPeaks();
					}
				}
				else
				{
					...
					if (!Application.isPlaying)
					{
						...
						{
							(target as AudioAnalyzer).PastePeaks();
						}
					}                    
				}
			}            
			...
			{
				(target as AudioAnalyzer).ClearPeaks();
			}        
		}
	}
	#endif
