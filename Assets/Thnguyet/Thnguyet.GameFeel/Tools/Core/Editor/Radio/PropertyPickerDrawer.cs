...
{
	[CustomPropertyDrawer(typeof(PropertyPicker), true)]
	public class PropertyPickerDrawer : PropertyDrawer
	{
		...
		protected virtual void DrawLevelProgressBar(Rect position, float level, Color frontColor, Color negativeColor, PropertyPickerViewData viewData)
		{
			...
			float progressWidth = FeelMaths.Remap(progressLevel, 0f, 1f, 0f, fullProgressWidth);
		...
		protected virtual Type GetPropertyType(SerializedProperty property, PropertyPickerViewData viewData)
		{
			...
			FeelProperty tempProperty;

			tempProperty = FeelProperty.FindProperty(viewData._propertiesList[viewData._selectedPropertyIndex], property.FindPropertyRelative("TargetComponent").objectReferenceValue as Component, null, property.FindPropertyRelative("TargetObject").objectReferenceValue as ScriptableObject);
