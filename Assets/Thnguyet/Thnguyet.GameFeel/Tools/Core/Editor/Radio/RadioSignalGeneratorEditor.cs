...
{
	[CustomEditor(typeof(RadioSignalGenerator), true)]
	[CanEditMultipleObjects]
	public class RadioSignalGeneratorEditor : RadioSignalEditor
	{
		...
		protected Color _spectrumColor = FeelColors.Aqua;
		protected Color _spectrumBoxColor = FeelColors.AliceBlue;
		...
		protected override void OnEnable()
		{
			...
			_list = new FeelReorderableList(serializedObject.FindProperty("SignalList"));
		...
		private void OnListAdd(FeelReorderableList list)
		{
		...
		protected virtual void DrawRawSpectrum()
		{
			...
			_normalizedTime = FeelMaths.Remap((float)_signalTime, 0f, _duration.floatValue, 0f, 1f);
			...
			if (_animatedPreview.boolValue)
			{
				...
				zeroX = maxX - FeelMaths.Remap(_normalizedTime, 0f, 1f, _spectrumBoxPosition.x + _externalMargin, _spectrumBoxPosition.x + _spectrumBoxSize.x);
				oneX = zeroX - 10;
			}
			...
			if (_duration.floatValue > 0f)
			{
				...
				if (Application.isPlaying)
				{
					boxSpectrumValue = FeelMaths.Remap(_radioSignal.GraphValue(_driverTime.floatValue), 0f, 1f, 0f, _spectrumBoxSize.y);
				}
				else
				{
					boxSpectrumValue = FeelMaths.Remap(_radioSignal.GraphValue(_normalizedTime), 0f, 1f, 0f, _spectrumBoxSize.y);
				}
			...
			if (Application.isPlaying && !_animatedPreview.boolValue)
			{
				_rect.x = _spectrumBoxPosition.x 
				          + FeelMaths.Remap(_driverTime.floatValue, 0f, 1f, 0f, _spectrumBoxSize.x);
			...
			{
				float xPosition = _spectrumBoxPosition.x + _externalMargin + FeelMaths.Remap(i, 0, _spectrumPointsCount, 0f, _spectrumBoxSize.x - _externalMargin * 2);
				...
				float spectrumValue = FeelMaths.Remap(_radioSignal.GraphValue(time), 0f, 1f, 0f, _spectrumBoxSize.y);
				float spectrumValuePrevious = FeelMaths.Remap(_radioSignal.GraphValue(timePrevious), 0f, 1f, 0f, _spectrumBoxSize.y);
