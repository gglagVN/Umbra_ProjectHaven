...
{	
	...
	public static class FeelDebug 
	{
						...
						.SelectMany(
							m => m.GetTypes().SelectMany(
								n => n.GetMethods(BindingFlags.Static | BindingFlags.NonPublic | BindingFlags.Public)
									.Where(o => o.GetCustomAttribute<DebugLogCommandAttribute>() != null))).ToArray();
				}
		...
		public static void DebugLogCommand(string command)
		{
			...
			string commandFirst = FeelString.UppercaseFirst(splitCommand[0]);
			...
			{ 
				...
				DebugLogCommandArgumentCountAttribute argumentAttribute = commandInfo.GetCustomAttributes<DebugLogCommandArgumentCountAttribute>(true).FirstOrDefault();
		...
		private static void LogCommand(string command, string color)
		{
			...
			LogHistory.Add(item);
			DebugLogEvent.Trigger(new DebugLogItem(null, "", Time.frameCount, Time.time, 0, false));
		}
		...
		public static string LogHistoryText
		{
			get
			{
				...
				for (int i = 0; i < LogHistory.Count; i++)
				{
					...
					log.Append("<color=#f9a682>[" + FeelTime.FloatToTimeString(LogHistory[i].Time, false, true, true, true) + "]</color> ");
		...
		#if GAMEFEEL_UI
		//public static FeelConsole _console;
		public static DebugOnScreenConsole _console;
		private const string _debugConsolePrefabPath = "DebugOnScreenConsole";
                
		/// <summary>
		/// Instantiates a FeelConsole if there isn't one already, and adds the message in parameter to it.
		/// </summary>
		/// <param name="message">Message.</param>
		...
		/// <summary>
		/// Instantiates a FeelConsole if there isn't one already, and displays the label in bold and its value next to it.
		/// </summary>
		...
		public static void InstantiateOnScreenConsole(int fontSize=25)
		{
			...
			if (_console == null)
			{
				...
				_console = (DebugOnScreenConsole) Object.FindAnyObjectByType(typeof(DebugOnScreenConsole));
			}
			...
			{	
				...
				loaded.name = "DebugOnScreenConsole";
				_console = loaded.GetComponent<DebugOnScreenConsole>();                
			}
		}
		...
		public static void SetOnScreenConsole(DebugOnScreenConsole newConsole)
		{
			_console = newConsole;
		}
		#endif
		...
		public static void DrawRectangle (Rect rectangle, Color color)
		{
			...
			FeelDebug.DrawRectangle (pos, color, scale); 
		}	
