...
{	
	...
	/// </summary>
	[AddComponentMenu("Thnguyet/GameFeel/Tools/Object Pool/Multiple Object Pooler")]
	public class MultipleObjectPooler : ObjectPooler
	{
		...
		[Information("If you set CanPoolSameObjectTwice to false, the Pooler will try to prevent the same object from being pooled twice to avoid repetition. This will only affect random pooling methods, not ordered pooling.",Thnguyet.GameFeel.InformationAttribute.InformationType.Info,false)]
		...
		/// a unique name that should match on all MultipleObjectPoolers you want to use together
		[Tooltip("a unique name that should match on all MultipleObjectPoolers you want to use together")]
		[Condition("MutualizeWaitingPools", true)]
		...
		[Condition("CanPoolSameObjectTwice", true, true)]
		public float OverflowAmount = 10f;
		
		public virtual List<MultipleObjectPooler> Owner { get; set; }
		...
		protected virtual GameObject AddOneObjectToThePool(GameObject typeOfObject)
		{
			...
			GameObject newGameObject = (GameObject)GameObjectExtensions.InstantiateDisabled(typeOfObject);
