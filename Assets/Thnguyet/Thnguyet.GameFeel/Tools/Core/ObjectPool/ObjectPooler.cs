...
{	
	...
	public abstract class ObjectPooler : MonoBehaviour
	{
		...
		protected FeelObjectPool _objectPool;
		protected const int _initialPoolsListCapacity = 5;
		protected bool _onSceneLoadedRegistered = false;
        
		public static List<FeelObjectPool> _pools = new List<FeelObjectPool>(_initialPoolsListCapacity);
		...
		public static void AddPool(FeelObjectPool pool)
		{
			if (_pools == null)
			{
				_pools = new List<FeelObjectPool>(_initialPoolsListCapacity);    
			}
		...
		/// </summary>
		/// <param name="pool"></param>
		public static void RemovePool(FeelObjectPool pool)
		{
			_pools?.Remove(pool);
		}
		...
		protected virtual bool CreateWaitingPool()
		{
			if (!MutualizeWaitingPools)
			{
				...
				_objectPool = _waitingPool.AddComponent<FeelObjectPool>();
			...
			{
				FeelObjectPool objectPool = ExistingPool(DetermineObjectPoolName());
				if (objectPool != null)
				{
		...
		/// <returns></returns>
		public virtual FeelObjectPool ExistingPool(string poolName)
		{
			if (_pools == null)
			{
				_pools = new List<FeelObjectPool>(_initialPoolsListCapacity);    
			...
			if (_pools.Count == 0)
			{
				var pools = FindObjectsByType<FeelObjectPool>(FindObjectsSortMode.None);
				if (pools.Length > 0)
				{
					_pools.AddRange(pools);
				}
			}
			foreach (FeelObjectPool pool in _pools)
			{
