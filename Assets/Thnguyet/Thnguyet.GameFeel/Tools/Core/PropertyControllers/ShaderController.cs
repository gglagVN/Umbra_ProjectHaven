...
namespace Thnguyet.GameFeel
{
	...
	[AddComponentMenu("Thnguyet/GameFeel/Tools/Property Controllers/Shader Controller")]
	public class ShaderController : FeelMonoBehaviour
	{
		...
		[Condition("UseMaterialPropertyBlocks", true)]
		...
		[Condition("RemapNoiseValues", true)]
		...
		{
			DrivenLevel = FeelMaths.Remap(normalizedLevel, 0f, 1f, remapZero, remapOne);
		}
		...
		protected virtual void UpdateValue()
		{
			...
			switch (ControlMode)
			{
				case ControlModes.PingPong:
					...
					CurrentValue = FeelTween.Tween(PingPong, 0f, Duration, MinValue, MaxValue, Curve);
					CurrentValueNormalized = FeelMaths.Remap(CurrentValue, MinValue, MaxValue, 0f, 1f);
					break;
				case ControlModes.Loop:
					...
					CurrentValue = FeelTween.Tween(LoopTime, 0f, LoopDuration, LoopStartValue, LoopEndValue, LoopCurve);
					CurrentValueNormalized = FeelMaths.Remap(CurrentValue, LoopStartValue, LoopEndValue, 0f, 1f);
					break;
				case ControlModes.Random:
					...
					if (RemapNoiseValues)
					{
						...
						CurrentValue = FeelMaths.Remap(CurrentValue, 0f, 1f, RemapNoiseZero, RemapNoiseOne);
					}
					else
					{
				...
				case ControlModes.OneTime:
					...
					_remappedTimeSinceStart = FeelMaths.Remap(GetTime() - _startedTimestamp, 0f, OneTimeDuration, 0f, 1f);
					CurrentValueNormalized = OneTimeCurve.Evaluate(_remappedTimeSinceStart);
					CurrentValue = FeelMaths.Remap(CurrentValueNormalized, 0f, 1f, OneTimeRemapMin, OneTimeRemapMax);
				...
				case ControlModes.ToDestination:
					...
					_remappedTimeSinceStart = FeelMaths.Remap(GetTime() - _startedTimestamp, 0f, ToDestinationDuration, 0f, 1f);
					...
					CurrentValueNormalized = FeelMaths.Remap(CurrentValue, _initialValue, ToDestinationValue, 0f, 1f);
					break;
			}
		...
		protected virtual void SetOneTimeFinalValue()
		{
			...
			else
			{
				...
				CurrentValue = FeelMaths.Remap(CurrentValue, 0f, 1f, OneTimeRemapMin, OneTimeRemapMax);
				CurrentValue *= OneTimeAmplitude;
				if (AddToInitialValue)
				{
					CurrentValue += InitialValue;
