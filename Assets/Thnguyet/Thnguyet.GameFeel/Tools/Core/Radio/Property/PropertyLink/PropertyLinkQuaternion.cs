...
namespace Thnguyet.GameFeel
{
	...
	/// </summary>
	public class PropertyLinkQuaternion : PropertyLink
	{
		...
		public override float GetLevel(PropertyEmitter emitter, FeelProperty property)
		{
			...
			axisValue = FeelMaths.Clamp(axisValue, emitter.QuaternionRemapMinToZero, emitter.QuaternionRemapMaxToOne, emitter.ClampMin, emitter.ClampMax);

			float returnValue = FeelMaths.Remap(axisValue, emitter.QuaternionRemapMinToZero, emitter.QuaternionRemapMaxToOne, 0f, 1f);
		...
		public override void SetLevel(PropertyReceiver receiver, FeelProperty property, float level)
		{
			...
			if (receiver.ModifyX)
			{
				float newX = FeelMaths.Remap(level, 0f, 1f, receiver.QuaternionRemapZero.x, receiver.QuaternionRemapOne.x);
				_newValue = _newValue * Quaternion.AngleAxis(newX, Vector3.right);
			}

			if (receiver.ModifyY)
			{
				float newY = FeelMaths.Remap(level, 0f, 1f, receiver.QuaternionRemapZero.y, receiver.QuaternionRemapOne.y);
				_newValue = _newValue * Quaternion.AngleAxis(newY, Vector3.up);
			}

			if (receiver.ModifyZ)
			{
				float newZ = FeelMaths.Remap(level, 0f, 1f, receiver.QuaternionRemapZero.z, receiver.QuaternionRemapOne.z);
		...
		/// <param name="property"></param>
		/// <returns></returns>
		protected virtual Quaternion GetValueOptimized(FeelProperty property)
		{
		...
		protected virtual void SetValueOptimized(FeelProperty property, Quaternion newValue)
		{
