...
namespace Thnguyet.GameFeel
{
	...
	/// </summary>
	public class PropertyLinkVector2 : PropertyLink
	{
		...
		public override float GetLevel(PropertyEmitter emitter, FeelProperty property)
		{
			...
			{
				case PropertyEmitter.Vector2Options.X:
					newValue = _vector2.x;
					break;
				case PropertyEmitter.Vector2Options.Y:
					newValue = _vector2.y;
					break;
			}
		...
		public override void SetLevel(PropertyReceiver receiver, FeelProperty property, float level)
		{
			...
			_newValue.x = receiver.ModifyX ? FeelMaths.Remap(level, 0f, 1f, receiver.Vector2RemapZero.x, receiver.Vector2RemapOne.x) : 0f;
			_newValue.y = receiver.ModifyY ? FeelMaths.Remap(level, 0f, 1f, receiver.Vector2RemapZero.y, receiver.Vector2RemapOne.y) : 0f;
		...
		/// <param name="property"></param>
		/// <returns></returns>
		protected virtual Vector2 GetValueOptimized(FeelProperty property)
		{
		...
		protected virtual void SetValueOptimized(FeelProperty property, Vector2 newValue)
		{
			if (_getterSetterInitialized)
			{
				SetVector2Delegate(_newValue);
