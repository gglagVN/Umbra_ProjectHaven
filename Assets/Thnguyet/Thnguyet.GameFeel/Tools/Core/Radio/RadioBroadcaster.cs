...
{
	/// <summary>
	/// A class used to broadcast a level to RadioReceiver(s), either directly or via events
	/// It can read from pretty much any value on any class
	/// </summary>
	[RequiresConstantRepaint]
	public class RadioBroadcaster : FeelMonoBehaviour
	{
		[Header("Source")]
		...
		public RadioReceiver[] Receivers;
		...
		[Condition("BroadcastOnChannel", true)]
		...
		protected virtual void ProcessBroadcast()
		{
			...
			if (level != _levelLastFrame)
			{
				...
				foreach (RadioReceiver receiver in Receivers)
				{
				...
				{
					RadioLevelEvent.Trigger(Channel, level);
				}
			}           
