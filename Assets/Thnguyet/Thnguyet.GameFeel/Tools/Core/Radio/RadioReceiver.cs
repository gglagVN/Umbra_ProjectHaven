...
{
	/// <summary>
	/// A class used to receive level values from a RadioBroadcaster, and apply it to (almost) any value on any object
	/// </summary>
	[RequiresConstantRepaint]
	public class RadioReceiver : FeelMonoBehaviour
	{
		[Header("Target")]
		/// the receiver to write the level to
		public PropertyReceiver Receiver;

		[Header("Channel")]
		/// whether or not this receiver should listen to the channel
		public bool CanListen = true;
		/// the Channel to listen to (has to match the one on the RadioBroadcaster you're listening to)
		[Condition("CanListen", true)]
		...
		public virtual void StartListening()
		{
			...
			RadioLevelEvent.Register(OnRadioLevelEvent);
		}
		...
		{
			_listeningToEvents = false;
			RadioLevelEvent.Unregister(OnRadioLevelEvent);
		}
	}
}