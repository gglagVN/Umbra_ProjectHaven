...
{
	[System.Serializable]
	public class RadioSignalOnValueChange : UnityEvent<float> { }

	/// <summary>
	/// A class used to define a signal, meant to be broadcasted by a RadioBroadcaster
	/// It'll output a Level value to broadcast, using one time, persistent or driven modes    
	/// Meant to be extended
	/// </summary>
	public abstract class RadioSignal : MonoBehaviour
	{
		...
		/// the level, to read from a RadioBroadcaster
		...
		public RadioSignalOnValueChange OnValueChange;
		...
		protected virtual void Update()
		{
			...
			else if (SignalMode == SignalModes.Persistent)
			{
				...
				DriverTime = FeelMaths.Remap(_signalTime, 0f, Duration, 0f, 1f);
			}
		...
		public virtual float ApplyBias(float t, float bias)
		{
			...
			bias = FeelMaths.Remap(bias, 0f, 1f, 1f, 0f);
