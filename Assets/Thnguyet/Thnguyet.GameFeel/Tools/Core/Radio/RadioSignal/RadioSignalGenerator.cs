...
namespace Thnguyet.GameFeel
{
	...
	/// You can then use these values from a RadioBroadcaster, or simply evaluate its value to use wherever you want, 
	/// like a supercharged animation curve. In that case, simply disable the component, and read from it using its Evaluate method
	/// </summary>
	public class RadioSignalGenerator : RadioSignal
	{
		...
		void Reset()
		{
			SignalList = new RadioSignalGeneratorItemList(){
				new RadioSignalGeneratorItem()
			};
		}
		...
		public virtual float Evaluate(float time)
		{
			...
			for (int i = 0; i < SignalList.Count; i++)
			{
				...
				{
					float newLevel = FeelSignal.GetValueNormalized(time,
					...
					level = (SignalList[i].Mode == RadioSignalGeneratorItem.GeneratorItemModes.Multiply) ? level * newLevel : level + newLevel;
		...
		protected override void Shake()
		{
			...
			if (SignalMode == SignalModes.OneTime)
			{
				...
				CurrentLevel = Evaluate(FeelMaths.Remap(elapsedTime, 0f, Duration, 0f, 1f));   
			}
			else
			{
	...
	/// </summary>
	[System.Serializable]
	public class RadioSignalGeneratorItem
	{
		...
		public FeelSignal.SignalType SignalType = FeelSignal.SignalType.Sine;
		/// if the type is animation curve, the curve to consider
		[EnumCondition("SignalType", (int)FeelSignal.SignalType.AnimationCurve)]
		public AnimationCurve Curve = new AnimationCurve(new Keyframe(0, 0), new Keyframe(1, 1));
		/// if the type is FeelTween, the tween to consider
		[EnumCondition("SignalType", (int)FeelSignal.SignalType.FeelTween)]
		public FeelTween.TweenCurve TweenCurve = FeelTween.TweenCurve.EaseInOutQuartic;
	...
	/// </summary>
	[System.Serializable]
	public class RadioSignalGeneratorItemList : ReorderableArray<RadioSignalGeneratorItem>
	{
	}
}
