...
namespace Thnguyet.GameFeel
{
	...
	/// You can create your own classes that implement this interface, or use the FeelPersistent class that comes with this package
	...
	/// </summary>
	public class PersistenceManager : PersistentSingleton<PersistenceManager>, EventListener<FeelGameEvent>
	{
		[Header("Persistence")]
		...
		public string PersistenceID = "Persistency";
		...
		[InspectorButton("SaveToMemory")]
		public bool SaveToMemoryButton;
		[InspectorButton("LoadFromMemory")]
		public bool LoadFromMemoryButton;
		[InspectorButton("SaveFromMemoryToFile")]
		public bool SaveToFileButton;
		[InspectorButton("LoadFromFileToMemory")]
		public bool LoadFromFileButton;
		[InspectorButton("DeletePersistenceFile")]
		...
		public static string _saveFolderName = "Tools/";
		...
		#region SAVE_AND_LOAD
			...
			public virtual void SaveToMemory()
			{
				...
				PersistenceSceneData sceneData = new PersistenceSceneData();
				...
				PersistenceEvent.Trigger(PersistenceEventType.DataSavedToMemory, PersistenceID);
			}
			...
			public virtual void LoadFromMemory()
			{
				...
				if (!SceneDatas.TryGetValue(_currentSceneName, out PersistenceSceneData sceneData))
				{
					return;
				}
				...
				PersistenceEvent.Trigger(PersistenceEventType.DataLoadedFromMemory, PersistenceID);
			}
			...
			{
				PersistenceManagerData saveData = new PersistenceManagerData();
				saveData.PersistenceID = PersistenceID;
				saveData.SaveDate = DateTime.Now.ToString();
				saveData.SceneDatas = SceneDatas;
				SaveLoadManager.Save(saveData, DetermineSaveName(), _saveFolderName);
				
				PersistenceEvent.Trigger(PersistenceEventType.DataSavedFromMemoryToFile, PersistenceID);
			}
			...
			{
				PersistenceManagerData saveData = (PersistenceManagerData)SaveLoadManager.Load(typeof(PersistenceManagerData), DetermineSaveName(), _saveFolderName);
				if ((saveData != null) && (saveData.SceneDatas != null))
				{
					SceneDatas = new DictionaryStringSceneData();
					SceneDatas = saveData.SceneDatas;	
				}
				PersistenceEvent.Trigger(PersistenceEventType.DataLoadedFromFileToMemory, PersistenceID);
			}
		...
		#region RESET
			...
			{
				SaveLoadManager.DeleteSave(DetermineSaveName(), _saveFolderName);
		...
		#region EVENTS
			...
			/// <summary>
			/// When we get a Event, we filter on its name and invoke the appropriate methods if needed
			/// </summary>
			...
			/// <summary>
			/// On enable, we start listening for GameEvents
			/// </summary>
			...
			/// <summary>
			/// On enable, we stop listening for GameEvents
			/// </summary>
