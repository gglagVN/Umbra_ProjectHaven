...
{	
	...
	/// </summary>
	[Serializable]
	public class AdditiveSceneLoadingManagerSettings
	{
		/// the possible ways to unload scenes
		public enum UnloadMethods { None, ActiveScene, AllScenes };
		/// the name of the SceneLoadingManager scene you want to use when in additive mode
		[Tooltip("the name of the SceneLoadingManager scene you want to use when in additive mode")]
		...
		public TweenType EntryFadeTween = null;
		...
		public TweenType ExitFadeTween = null;
	...
	/// </summary>
	[Serializable]
	public class SceneLoadingSpeedInterval
	{
		/// The progress interval (between 0 and 1) 
		public FeelInterval<float> Interval;
	...
	/// This is a new version of the classic LoadingSceneManager (now renamed to SceneLoadingManager for consistency)
	/// </summary>
	public class AdditiveSceneLoadingManager : FeelMonoBehaviour 
	{
		...
		[InspectorGroup("Audio Listener", true, 3)]
		public AudioListener LoadingAudioListener;
		
		[InspectorGroup("Settings", true, 10)]
		...
		[InspectorGroup("Progress Events", true, 11)]
		...
		[InspectorGroup("State Events", true, 12)]
		...
		protected static List<SceneLoadingSpeedInterval> _speedIntervals;
		protected static float _beforeEntryFadeDelay;
		protected static TweenType _entryFadeTween;
		...
		protected static TweenType _exitFadeTween;
		...
		protected SceneLoadingAntiSpill _antiSpill = new SceneLoadingAntiSpill();
		...
		protected static void InitializeStatics()
		{
			...
			_speedIntervals = new List<SceneLoadingSpeedInterval>();
		...
		public static void LoadScene(string sceneToLoadName, AdditiveSceneLoadingManagerSettings settings)
		{
		...
		public static void LoadScene(string sceneToLoadName, string loadingSceneName = "AdditiveLoadingScreen", 
			...
			TweenType entryFadeTween = null, TweenType exitFadeTween = null,
			float progressBarSpeed = 5f, 
			FadeModes fadeMode = FadeModes.FadeInThenOut,
			AdditiveSceneLoadingManagerSettings.UnloadMethods unloadMethod = AdditiveSceneLoadingManagerSettings.UnloadMethods.AllScenes,
			string antiSpillSceneName = "",
			List<SceneLoadingSpeedInterval> speedIntervals = null,
			bool debugMode = false)
		{
			if (_loadingInProgress)
			{
				Debug.LogError("LoadingSceneManagerAdditive : a request to load a new scene was emitted while a scene load was already in progress");  
				return;
			}

			if (entryFadeTween == null)
			{
				entryFadeTween = new TweenType(FeelTween.TweenCurve.EaseInOutCubic);
			}

			if (exitFadeTween == null)
			{
				exitFadeTween = new TweenType(FeelTween.TweenCurve.EaseInOutCubic);
			}

			if (secureLoad)
			{
				_scenesInBuild = FeelScene.GetScenesInBuild();
	            
				if (!_scenesInBuild.Contains(sceneToLoadName))
				{
					Debug.LogError("LoadingSceneManagerAdditive : impossible to load the '"+sceneToLoadName+"' scene, " +
					               "there is no such scene in the project's build settings.");
					return;
				}
				if (!_scenesInBuild.Contains(loadingSceneName))
				{
					Debug.LogError("LoadingSceneManagerAdditive : impossible to load the '"+loadingSceneName+"' scene, " +
					               "there is no such scene in the project's build settings.");
					return;
				}
			}
		...
		private static Scene[] GetScenesToUnload(AdditiveSceneLoadingManagerSettings.UnloadMethods unloaded)
		{
	        
			switch (unloaded) {
				case AdditiveSceneLoadingManagerSettings.UnloadMethods.None:
					_initialScenes = new Scene[0];
					break;
				case AdditiveSceneLoadingManagerSettings.UnloadMethods.ActiveScene:
					_initialScenes = new Scene[1] {SceneManager.GetActiveScene()};
					break;
				default:
				case AdditiveSceneLoadingManagerSettings.UnloadMethods.AllScenes:
					_initialScenes = FeelScene.GetLoadedScenes();
					break;
			}
		...
		{
			LoadingSceneDebug("LoadingSceneManagerAdditive : Initialization");

			if (DebugMode)
			{
				foreach (Scene scene in _initialScenes)
				{
					LoadingSceneDebug("LoadingSceneManagerAdditive : Initial scene : " + scene.name);
				}    
			}
		...
		protected virtual void UpdateProgress()
		{
			...
			{
				_interpolatedLoadProgress = FeelMaths.Approach(_interpolatedLoadProgress, _loadProgress, Time.unscaledDeltaTime * ComputeInterpolationSpeed(_interpolatedLoadProgress));
		...
		public static float ComputeInterpolationSpeed(float t) 
		{
			...
			{
				foreach (SceneLoadingSpeedInterval interval in _speedIntervals)
				{
		...
		protected virtual void InitiateLoad()
		{
			...
			LoadingSceneDebug("LoadingSceneManagerAdditive : Initiate Load");
		...
		protected virtual IEnumerator ProcessDelayBeforeEntryFade()
		{
			...
			{
				LoadingSceneDebug("LoadingSceneManagerAdditive : delay before entry fade, duration : " + _beforeEntryFadeDelay);
				...
				yield return FeelCoroutine.WaitForUnscaled(_beforeEntryFadeDelay);
			}
		}
		...
		protected virtual IEnumerator EntryFade()
		{
			...
			{
				LoadingSceneDebug("LoadingSceneManagerAdditive : starting entry fade, duration : " + _entryFadeDuration);
				...
				{
					yield return null;
					FadeOutEvent.Trigger(_entryFadeDuration, _entryFadeTween, FaderID, true);
				}
				else
				{
					yield return null;
					FadeInEvent.Trigger(_entryFadeDuration, _entryFadeTween, FaderID, true);
				}           

				yield return FeelCoroutine.WaitForUnscaled(_entryFadeDuration);
			}
		}
		...
		protected virtual IEnumerator ProcessDelayAfterEntryFade()
		{
			...
			{
				LoadingSceneDebug("LoadingSceneManagerAdditive : delay after entry fade, duration : " + _afterEntryFadeDelay);
				...
				yield return FeelCoroutine.WaitForUnscaled(_afterEntryFadeDelay);
			}
		}
		...
		protected virtual IEnumerator UnloadOriginScenes()
		{
			foreach (Scene scene in _initialScenes)
			{
				LoadingSceneDebug("LoadingSceneManagerAdditive : unload scene " + scene.name);
				...
				{
					Debug.LogWarning("LoadingSceneManagerAdditive : invalid scene : " + scene.name);
					continue;
				}
		...
		{
			LoadingSceneDebug("LoadingSceneManagerAdditive : load destination scene");
			...
			LoadingSceneDebug("LoadingSceneManagerAdditive : load progress complete");
			...
			LoadingSceneDebug("LoadingSceneManagerAdditive : interpolated load complete");
		...
		protected virtual IEnumerator ProcessDelayBeforeSceneActivation()
		{
			...
			{
				LoadingSceneDebug("LoadingSceneManagerAdditive : delay before scene activation, duration : " + _beforeSceneActivationDelay);
				SceneLoadingManager.LoadingSceneEvent.Trigger(_sceneToLoadName, SceneLoadingManager.LoadingStatus.BeforeSceneActivation);
				OnBeforeSceneActivation?.Invoke();
				
				yield return FeelCoroutine.WaitForUnscaled(_beforeSceneActivationDelay);
			}
		...
		{
			yield return FeelCoroutine.WaitForFrames(1);
			_loadDestinationAsyncOperation.allowSceneActivation = true;
			while (_loadDestinationAsyncOperation.progress < 1.0f)
			{
				yield return null;
			}
			LoadingSceneDebug("LoadingSceneManagerAdditive : activating destination scene");
		...
		protected virtual IEnumerator ProcessDelayAfterSceneActivation()
		{
			...
			{
				LoadingSceneDebug("LoadingSceneManagerAdditive : delay after scene activation, duration : " + _afterSceneActivationDelay);
				SceneLoadingManager.LoadingSceneEvent.Trigger(_sceneToLoadName, SceneLoadingManager.LoadingStatus.AfterSceneActivation);
				OnAfterSceneActivation?.Invoke();
				
				yield return FeelCoroutine.WaitForUnscaled(_afterSceneActivationDelay);
			}
		}
		...
		protected virtual IEnumerator ExitFade()
		{
			...
			{
				LoadingSceneDebug("LoadingSceneManagerAdditive : starting exit fade, duration : " + _exitFadeDuration);
				SceneLoadingManager.LoadingSceneEvent.Trigger(_sceneToLoadName, SceneLoadingManager.LoadingStatus.ExitFade);
				OnExitFade?.Invoke();
				
				if (_fadeMode == FadeModes.FadeOutThenIn)
				{
					FadeInEvent.Trigger(_exitFadeDuration, _exitFadeTween, FaderID, true);
				}
				else
				{
					FadeOutEvent.Trigger(_exitFadeDuration, _exitFadeTween, FaderID, true);
				}
				yield return FeelCoroutine.WaitForUnscaled(_exitFadeDuration);
			}
		}
		...
		protected virtual void OnLoadOperationComplete(AsyncOperation obj)
		{
			...
			LoadingSceneDebug("LoadingSceneManagerAdditive : set active scene to " + _sceneToLoadName);
		...
		{
			LoadingSceneDebug("LoadingSceneManagerAdditive : unloading scene loader");
		...
		protected virtual void LoadingSceneDebug(string message)
		{
			...
			output += "<color=#f9a682>[" + FeelTime.FloatToTimeString(Time.time, false, true, true, true) + "]</color> ";
			output +=  message;
			FeelDebug.DebugLogInfo(output);
		}
	}
}
