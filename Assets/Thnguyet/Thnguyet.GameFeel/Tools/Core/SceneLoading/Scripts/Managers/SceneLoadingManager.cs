...
{	
	...
	/// This class used to be known as LoadingSceneManager, and has now been renamed to SceneLoadingManager for consistency
	/// </summary>
	public class SceneLoadingManager : MonoBehaviour 
	{
		...
		protected static TweenType _tween;
		...
		{
			_tween = new TweenType(FeelTween.TweenCurve.EaseOutCubic);
			#if GAMEFEEL_UI
		...
		protected virtual void Update()
		{
			Time.timeScale = 1f;
			#if GAMEFEEL_UI
				_progressBarImage.fillAmount = FeelMaths.Approach(_progressBarImage.fillAmount,_fillTarget,Time.deltaTime*ProgressBarSpeed);
			#endif
		}
		...
		protected virtual IEnumerator LoadAsynchronously() 
		{
			...
			// we fade from black
			FadeOutEvent.Trigger(StartFadeDuration, _tween);
			yield return FeelCoroutine.WaitFor(StartFadeDuration);
			...
			LoadingComplete();
			yield return FeelCoroutine.WaitFor(LoadCompleteDelay);

			// we fade to black
			FadeInEvent.Trigger(ExitFadeDuration, _tween);
			yield return FeelCoroutine.WaitFor(ExitFadeDuration);
		...
		protected virtual void LoadingComplete() 
		{
			...
			#if GAMEFEEL_UI
			StartCoroutine(FeelFade.FadeCanvasGroup(LoadingProgressBar,0.1f,0f));
			StartCoroutine(FeelFade.FadeCanvasGroup(LoadingAnimation,0.1f,0f));
			StartCoroutine(FeelFade.FadeCanvasGroup(LoadingCompleteAnimation,0.1f,1f));
			#endif
		}
	}
}
