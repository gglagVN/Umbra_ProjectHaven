...
namespace Thnguyet.GameFeel
{
	public struct StateChangeEvent<T> where T: struct, IComparable, IConvertible, IFormattable
	{
		...
		public FeelStateMachine<T> TargetStateMachine;
		public T NewState;
		public T PreviousState;

		public StateChangeEvent(FeelStateMachine<T> stateMachine)
		{
	...
	/// </summary>
	public class FeelStateMachine<T> : IStateMachine where T : struct, IComparable, IConvertible, IFormattable
	{
		...
		/// let's assume in some class we have a public FeelStateMachine<CharacterStates.MovementStates> MovementState, and we use that to track the state of a moving character (idle, walking, running etc)
		/// in any other class, we could do :
		/// public class TestListener : MonoBehaviour, EventListener<StateChangeEvent<CharacterStates.MovementStates>>
		/// {
		/// 	// triggered every time a state change event occurs
		/// 	public void OnMMEvent(StateChangeEvent<CharacterStates.MovementStates> stateChangeEvent)
		/// 	{
		...
		/// 	{
		/// 		FeelEventManager.AddListener<StateChangeEvent<CharacterStates.MovementStates>>(this);
		/// 	}
		/// 
		/// 	private void OnDisable() // on disable we stop listening for these events
		/// 	{
		/// 		FeelEventManager.RemoveListener<StateChangeEvent<CharacterStates.MovementStates>>(this);
		/// 	}
		/// }
		...
		public FeelStateMachine(GameObject target, bool triggerEvents)
		{
			this.Target = target;
		...
		public virtual void ChangeState(T newState)
		{
			...
			if (TriggerEvents)
			{
				FeelEventManager.TriggerEvent (new StateChangeEvent<T> (this));
			}
		}
