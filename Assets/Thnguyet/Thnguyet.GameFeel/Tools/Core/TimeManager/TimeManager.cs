...
namespace Thnguyet.GameFeel.Feedbacks
{
	...
	/// </summary>
	public enum TimeScaleLerpModes { Speed, Duration, NoInterpolation } 
	...
	public struct TimeScaleProperties
	{
		...
		public TweenType TimeScaleLerpCurve;
		public float TimeScaleLerpDuration;
		public bool TimeScaleLerpOnUnfreeze;
		public TweenType TimeScaleLerpCurveOnUnfreeze;
	...
	public struct TimeScaleEvent
	{
		...
		public delegate void Delegate(TimeScaleMethods timeScaleMethod, float timeScale, float duration, bool lerp, float lerpSpeed, bool infinite,
			TimeScaleLerpModes timeScaleLerpMode = TimeScaleLerpModes.Speed, TweenType timeScaleLerpCurve = null, float timeScaleLerpDuration = 0.2f, 
			bool timeScaleLerpOnUnfreeze = false, TweenType timeScaleLerpCurveOnUnfreeze = null, float timeScaleLerpDurationOnUnfreeze = 0.2f);
		...
		{
			OnEvent?.Invoke(TimeScaleMethods.Unfreeze, 0f, 0f, false, 0f, false);
		}
		
		static public void Reset()
		{
			OnEvent?.Invoke(TimeScaleMethods.Reset, 0f, 0f, false, 0f, false);
		}
	}
	...
	/// </summary>
	[AddComponentMenu("Thnguyet/GameFeel/Feedbacks/Shakers/Various/Time Manager")]
	public class TimeManager : FeelSingleton<TimeManager>
	{	
		...
		[InspectorButton("TestButtonToSlowDownTime")]
		...
		protected virtual void Update()
		{
			...
			{
				if (_currentProperty.TimeScaleLerpMode == TimeScaleLerpModes.Speed)
				{
				...
				else if (_currentProperty.TimeScaleLerpMode == TimeScaleLerpModes.Duration)
				{
					float timeSinceStart = Time.unscaledTime - _startedAt;

					if (timeSinceStart < _currentProperty.TimeScaleLerpDuration)
					{
						float progress = FeelMaths.Remap(timeSinceStart, 0f, _currentProperty.TimeScaleLerpDuration, 0f,
							1f);
						float delta = _currentProperty.TimeScaleLerpCurve.Evaluate(progress);
						float newValue = FeelMaths.Remap(delta, 0f, 1f, _initialTimeScale, TargetTimeScale);
		...
		protected virtual void SetTimeScale(TimeScaleProperties timeScaleProperties)
		{
			if (timeScaleProperties.TimeScaleLerp &&
			    timeScaleProperties.TimeScaleLerpMode == TimeScaleLerpModes.Duration)
			{
		...
		public virtual void Unfreeze()
		{
			...
			if (_timeScaleProperties.Count == 0)
			{
				if (_resetProperty.TimeScaleLerp && _resetProperty.TimeScaleLerpMode == TimeScaleLerpModes.Duration && _resetProperty.TimeScaleLerpOnUnfreeze)
				{
					_lerpingBackToNormal = true;
					TimeScaleEvent.Trigger(TimeScaleMethods.For, NormalTimeScale, _resetProperty.TimeScaleLerpDurationOnUnfreeze, _resetProperty.TimeScaleLerp, 
						_resetProperty.LerpSpeed, true, TimeScaleLerpModes.Duration, _resetProperty.TimeScaleLerpCurveOnUnfreeze, _resetProperty.TimeScaleLerpDurationOnUnfreeze);	
				}
				else
				{
					ResetTimeScale();	
		...
		/// </summary>
		/// <param name="timeScaleEvent">TimeScaleEvent event.</param>
		...
		void OnEnable()
		{
			FreezeFrameEvent.Register(OnMMFreezeFrameEvent);
		...
		void OnDisable()
		{
			FreezeFrameEvent.Unregister(OnMMFreezeFrameEvent);
