...
namespace Thnguyet.GameFeel
{
	...
	/// This asset will include curves (anti or not) from the FeelTween library, to use anywhere animation curves are required
	/// </summary>
	public class AnimationCurveGenerator : MonoBehaviour
	{
		[Header("Save settings")]
		/// the path to save the asset at
		public string AnimationCurveFilePath = "Assets/Tools/FeelTween/Editor/";
		/// the name of the asset
		public string AnimationCurveFileName = "Curves.curves";
		...
		[InspectorButton("GenerateAnimationCurvesAsset")]
		...
		public virtual void GenerateAnimationCurvesAsset()
		{
			...
			foreach (FeelTween.TweenCurve curve in Enum.GetValues(typeof(FeelTween.TweenCurve)))
			{
		...
		protected virtual void CreateAnimationCurve(ScriptableObject asset, FeelTween.TweenCurve curveType, int curveResolution, bool anti)
		{
			...
			for (int i = 0; i < curveResolution; i++)
			{
				...
				if (anti)
				{
					_keyframe.value = FeelTween.Tween(_keyframe.time, 0f, 1f, 1f, 0f, curveType);
				}
				else
				{
					_keyframe.value = FeelTween.Tween(_keyframe.time, 0f, 1f, 0f, 1f, curveType);
				}                
