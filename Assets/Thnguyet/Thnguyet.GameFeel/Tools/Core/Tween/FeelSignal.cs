...
namespace Thnguyet.GameFeel
{
	...
	public class FeelSignal
	{
		...
		/// <returns></returns>
		public static float GetValue(float time, SignalType signalType, float phase, float amplitude, float frequency, float offset, bool Invert = false, AnimationCurve curve = null, FeelTween.TweenCurve tweenCurve = FeelTween.TweenCurve.LinearTween)
		{
			float value = 0f;
			...
			{ 
				...
				case SignalType.AnimationCurve:
					...
					break;
				case SignalType.FeelTween:
					t = (t != 1f) ? t - Mathf.Floor(t) : 1f;
					value = FeelTween.Tween(t, 0f, 1f, 0f, 1f, tweenCurve);
					break;
			}
		...
		public static float GetValueNormalized(float time, SignalType signalType,
			...
			AnimationCurve curve = null, FeelTween.TweenCurve tweenCurve = FeelTween.TweenCurve.LinearTween,
			bool clamp = true, float clampMin = 0f, float clampMax = 1f, bool backAndForth = false, float backAndForthTippingPoint = 0.5f)
		{
			...
			if (backAndForth)
			{
				...
				{
					time = FeelMaths.Remap(time, 0f, backAndForthTippingPoint, 0f, 1f);
				}
				...
				{
					time = FeelMaths.Remap(time, backAndForthTippingPoint, 1f, 1f, 0f);
				}
			}
			...
			switch (signalType)
			{
				case SignalType.Sine:
					value = (float)Mathf.Sin(2f * Mathf.PI * t);
					value = FeelMaths.Remap(value, -1f, 1f, 0f, 1f);
					break;
				case SignalType.Square:
					value = Mathf.Sign(Mathf.Sin(2f * Mathf.PI * t));
					value = FeelMaths.Remap(value, -1f, 1f, 0f, 1f);
			...
			if (Invert)
			{
				value = FeelMaths.Remap(value, 0f, 1f, 1f, 0f);
			}
