...
namespace Thnguyet.GameFeel
{
	...
	public class FeelTween : MonoBehaviour
	{
		/// <summary>
		...
		{
			currentTime = FeelMaths.Remap(currentTime, initialTime, endTime, 0f, 1f);
			currentTime = TweenDelegateArray[(int)curve](currentTime);
			return startValue + currentTime * (endValue - startValue);
		}
		
		public static long Tween(float currentTime, float initialTime, float endTime, long startValue, long endValue, TweenCurve curve)
		{
			currentTime = FeelMaths.Remap(currentTime, initialTime, endTime, 0f, 1f);
			currentTime = TweenDelegateArray[(int)curve](currentTime);
		...
		{
			if (tweenType.TweenDefinitionType == TweenDefinitionTypes.FeelTween)
			{
		...
		/// <summary>
		/// To use :
		/// public FeelTween.TweenCurve Tween = FeelTween.TweenCurve.EaseInOutCubic;
		/// private FeelTween.TweenDelegate _tween;
		///
		/// _tween = FeelTween.GetTweenMethod(Tween);
		...
		public static Vector4 Tween(float currentTime, float initialTime, float endTime, Vector4 startValue, Vector4 endValue, TweenCurve curve)
		{
			startValue.x = Tween(currentTime, initialTime, endTime, startValue.x, endValue.x, curve);
			startValue.y = Tween(currentTime, initialTime, endTime, startValue.y, endValue.y, curve);
			startValue.z = Tween(currentTime, initialTime, endTime, startValue.z, endValue.z, curve);
		...
		public static long Tween(float currentTime, float initialTime, float endTime, long startValue, long endValue, TweenType tweenType)
		{
			if (tweenType.TweenDefinitionType == TweenDefinitionTypes.FeelTween)
			{
				return Tween(currentTime, initialTime, endTime, startValue, endValue, tweenType.TweenCurve);
		...
		public static Vector4 Tween(float currentTime, float initialTime, float endTime, Vector4 startValue, Vector4 endValue, TweenType tweenType)
		{
			if (tweenType.TweenDefinitionType == TweenDefinitionTypes.FeelTween)
			{
				return Tween(currentTime, initialTime, endTime, startValue, endValue, tweenType.TweenCurve);
		...
		public static Coroutine MoveTransform(MonoBehaviour mono, Transform targetTransform, Vector3 origin, Vector3 destination, 
			WaitForSeconds delay, float delayDuration, float duration, FeelTween.TweenCurve curve, bool ignoreTimescale = false)
		{
		...
		public static Coroutine MoveTransform(MonoBehaviour mono, Transform targetTransform, Transform origin, Transform destination, WaitForSeconds delay, float delayDuration, float duration, 
			FeelTween.TweenCurve curve, bool updatePosition = true, bool updateRotation = true, bool ignoreTimescale = false)
		{
		...
		public static Coroutine RotateTransformAround(MonoBehaviour mono, Transform targetTransform, Transform center, Transform destination, float angle, WaitForSeconds delay, float delayDuration, 
			float duration, FeelTween.TweenCurve curve, bool ignoreTimescale = false)
		{
		...
		protected static IEnumerator MoveRectTransformCo(RectTransform targetTransform, Vector3 origin, Vector3 destination, WaitForSeconds delay,
			float delayDuration, float duration, FeelTween.TweenCurve curve, bool ignoreTimescale = false)
		{
			if (delayDuration > 0f)
			{
				yield return delay;
			...
			while (timeLeft > 0f)
			{
				targetTransform.localPosition = FeelTween.Tween(duration - timeLeft, 0f, duration, origin, destination, curve);
			...
			float delayDuration, float duration, FeelTween.TweenCurve curve, bool ignoreTimescale = false)
		{
			...
			while (timeLeft > 0f)
			{
				targetTransform.transform.position = FeelTween.Tween(duration - timeLeft, 0f, duration, origin, destination, curve);
				timeLeft -= ignoreTimescale ? Time.unscaledDeltaTime : Time.deltaTime;
				yield return null;
			}
			...
			FeelTween.TweenCurve curve, bool updatePosition = true, bool updateRotation = true, bool ignoreTimescale = false)
		{
			...
			while (timeLeft > 0f)
			{
				if (updatePosition)
				{
					targetTransform.transform.position = FeelTween.Tween(duration - timeLeft, 0f, duration, origin.position, destination.position, curve);
				}
				if (updateRotation)
				{
					targetTransform.transform.rotation = FeelTween.Tween(duration - timeLeft, 0f, duration, origin.rotation, destination.rotation, curve);
				}
				timeLeft -= ignoreTimescale ? Time.unscaledDeltaTime : Time.deltaTime;
				yield return null;
			}
		...
		protected static IEnumerator RotateTransformAroundCo(Transform targetTransform, Transform center, Transform destination, float angle, WaitForSeconds delay, float delayDuration, float duration, 
			FeelTween.TweenCurve curve, bool ignoreTimescale = false)
		{
			if (delayDuration > 0f)
			{
				yield return delay;
			...
			while (timeSpent < duration)
			{
				...
				float newAngle = FeelTween.Tween(timeSpent, 0f, duration, 0f, angle, curve);
