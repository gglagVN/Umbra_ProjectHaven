...
namespace Thnguyet.GameFeel
{
	...
	[AddComponentMenu("Thnguyet/GameFeel/Tools/GUI/Fader Directional")]
	public class FaderDirectional : FeelMonoBehaviour, EventListener<FadeEvent>, EventListener<FadeInEvent>, EventListener<FadeOutEvent>, EventListener<FadeStopEvent>
	{
		...
		[InspectorGroup("Directional Fader", true, 123)]
		...
		{
			FadeEvent.Trigger(DefaultDuration, 1f, DefaultTween, ID, IgnoreTimescale, this.transform.position);
		}
		...
		{
			FadeInEvent.Trigger(1f, DefaultTween, ID, IgnoreTimescale, this.transform.position);
		}
		...
		{
			FadeOutEvent.Trigger(1f, DefaultTween, ID, IgnoreTimescale, this.transform.position);
		}
		...
		protected virtual void Fade()
		{
			...
			{
				_newPosition = FeelTween.Tween(currentTime, _fadeStartedAt, endTime, _fromPosition, _toPosition, _currentCurve);
		...
		protected virtual IEnumerator StartFading(bool fadingIn, float duration, TweenType curve, int id,
			bool ignoreTimeScale, Vector3 worldPosition)
		{
			...
			if (InitialDelay > 0f)
			{
				yield return FeelCoroutine.WaitFor(InitialDelay);
			}
			...
			_newPosition = FeelTween.Tween(0f, 0f, duration, _fromPosition, _toPosition, _currentCurve);
			_rectTransform.anchoredPosition = _newPosition;
		}
