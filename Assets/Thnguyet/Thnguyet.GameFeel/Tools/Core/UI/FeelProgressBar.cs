...
namespace Thnguyet.GameFeel
{
	...
	[AddComponentMenu("Thnguyet/GameFeel/Tools/GUI/Progress Bar")]
	public class FeelProgressBar : FeelMonoBehaviour
	{
		#if GAMEFEEL_UI
		...
		[InspectorGroup("Bindings", true, 10)]
		...
		[InspectorGroup("Fill Settings", true, 11)]
		...
		[InspectorGroup("Foreground Bar Settings", true, 12)]
		...
		[InspectorGroup("Delayed Bar Decreasing", true, 13)]
		...
		[InspectorGroup("Delayed Bar Increasing", true, 18)]
		...
		/// the speed at which to lerp the delayed bar
		[Tooltip("the speed at which to lerp the delayed bar")]
		[Condition("LerpIncreasingDelayedBar", true)]
		...
		[Condition("ApplyBumpIntensityMultiplier", true)]
		...
		[InspectorGroup("Debug Read Only", true, 19)]
		...
		public virtual void UpdateBar(float currentValue,float minValue,float maxValue) 
		{
			...
			_newPercent = FeelMaths.Remap(currentValue, minValue, maxValue, MinimumBarFillValue, MaximumBarFillValue);
		...
		{
			float newPercent = FeelMaths.Remap(currentValue, minValue, maxValue, 0f, 1f);
			SetBar01(newPercent);
		}
		...
		public virtual void SetBar01(float newPercent)
		{
			...
			newPercent = FeelMaths.Remap(newPercent, 0f, 1f, MinimumBarFillValue, MaximumBarFillValue);
		...
		protected virtual float ComputeNewFill(bool lerpBar, float barSpeed, float barDuration, AnimationCurve barCurve, float delay, float lastPercent, out float t)
		{
			...
			if (lerpBar)
			{
				...
				delta = FeelMaths.Remap(timeSpent, 0f, duration, 0f, 1f);
		...
		protected virtual void SetBarInternal(float newAmount, Transform bar, Image image, Vector2 initialSize)
		{
			...
			switch (FillMode)
			{
				...
				case FillModes.Width:
					...
					float newSizeX = FeelMaths.Remap(newAmount, 0f, 1f, 0, initialSize.x);
				...
				case FillModes.Height:
					...
					float newSizeY = FeelMaths.Remap(newAmount, 0f, 1f, 0, initialSize.y);
		...
		{
			yield return FeelCoroutine.WaitFor(delay);
			this.gameObject.SetActive(false);
		}
