...
{	
	...
	public class FeelAchievement  
	{
		...
		{
			AchievementChangedEvent.Trigger(this);
		...
		/// </summary>
		public virtual FeelAchievement Copy()
		{
			FeelAchievement clone = new FeelAchievement ();
			// we use Json utility to store a copy of our achievement, not a reference
			clone = JsonUtility.FromJson<FeelAchievement>(JsonUtility.ToJson(this));
			return clone;
		}
	}
}