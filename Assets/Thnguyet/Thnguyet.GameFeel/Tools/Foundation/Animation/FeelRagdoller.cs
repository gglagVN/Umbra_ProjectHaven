...
namespace Thnguyet.GameFeel
{
	...
	/// If you have parts of your ragdoll that you don't want to be affected by this script (a weapon for example), just add a RagdollerIgnore component to them
	/// </summary>
	[AddComponentMenu("Thnguyet/GameFeel/Tools/Animation/Ragdoller")]
	public class FeelRagdoller : MonoBehaviour
	{
		/// <summary>
			...
			set
			{
				if (value == true)
				{
					...
					if (CurrentState == RagdollStates.Animated)
					{
						...
						AnimatorExtensions.UpdateAnimatorBool(_animator, _getUpFromBackAnimationParameter, false, _animatorParameters);
						AnimatorExtensions.UpdateAnimatorBool(_animator, _getUpFromBellyAnimationParameter, false, _animatorParameters);
					}
				}
				else
				{
					if (CurrentState == RagdollStates.Ragdolling)
					{
						...
						{
							AnimatorExtensions.UpdateAnimatorBool(_animator, _getUpFromBackAnimationParameter, true, _animatorParameters);
						}
						else
						{
							AnimatorExtensions.UpdateAnimatorBool(_animator, _getUpFromBellyAnimationParameter, true, _animatorParameters);
						}
					}
				}
			}
		...
		protected virtual void Initialization()
		{
			...
			{
				if (rigidbody.gameObject.GetComponentNoAlloc<RagdollerIgnore>() == null)
				{
		...
		protected virtual void RegisterAnimatorParameters()
		{
			...
			if (_animator.HasParameterOfType(_getUpFromBackAnimationParameterName, AnimatorControllerParameterType.Bool))
			{
				_animatorParameters.Add(_getUpFromBackAnimationParameter);
			}
			if (_animator.HasParameterOfType(_getUpFromBellyAnimationParameterName, AnimatorControllerParameterType.Bool))
			{
