...
namespace Thnguyet.GameFeel
{
	...
	/// Note that this component extends the TouchJoystick class so you don't need to add another joystick to it. It's both the detection zone and the stick itself.
	/// </summary>
	[AddComponentMenu("Thnguyet/GameFeel/Tools/Controls/Touch Follower Joystick")]
	public class TouchFollowerJoystick : TouchJoystick
	{
		[InspectorGroup("Follower Joystick", true, 23)]
		...
		[Condition("InterpolateFollowMovement", true)]
		...
		[Condition("SpringFollowInterpolation", true)]
		...
		[InspectorGroup("Background Constraints", true, 24)]
		...
		protected virtual void HandleMovementInterpolation()
		{
			...
			if (SpringFollowInterpolation)
			{
				...
				FeelMaths.Spring(ref _newBackgroundPosition, _backgroundPositionTarget, ref _springVelocity, SpringDamping, SpringFrequency, Time.unscaledDeltaTime);
				BackgroundCanvasGroup.transform.position = _newBackgroundPosition;
			}
			else
			{
				BackgroundCanvasGroup.transform.position = FeelMaths.Lerp(BackgroundCanvasGroup.transform.position, _backgroundPositionTarget, InterpolateFollowMovementSpeed, Time.unscaledDeltaTime);	
			}
		}
		...
		protected override void OnDrawGizmos()
		{
			...
			if (BackgroundConstraintRectTransform != null)
			{
				...
				FeelDebug.DrawGizmoPoint(_innerRectTransformBottomLeft, Color.cyan, gizmoSize);
				FeelDebug.DrawGizmoPoint(_innerRectTransformTopLeft, Color.cyan, gizmoSize);
				FeelDebug.DrawGizmoPoint(_innerRectTransformTopRight, Color.cyan, gizmoSize);
				FeelDebug.DrawGizmoPoint(_innerRectTransformBottomRight, Color.cyan, gizmoSize);
			}
		}
		#endif
	}
