...
namespace Thnguyet.GameFeel 
{
	...
	/// It's got an activation button, a run button, and an option to slow down time (this will require a TimeManager present in the scene)
	/// </summary>
	[AddComponentMenu("Thnguyet/GameFeel/Tools/Camera/Ghost Camera")]
	public class GhostCamera : MonoBehaviour
	{
		...
		[Condition("UseMobileControls", true)]
		...
		protected virtual void ToggleSlowMotion()
		{
			...
			if (TimeAltered)
			{
				TimeScaleEvent.Trigger(TimeScaleMethods.For, TimescaleModifier, 1f, true, 5f, true);
			}
			else
			{
